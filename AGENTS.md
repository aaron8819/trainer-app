...
- Retrospectives should only run when they provide meaningful signal. Avoid running on trivial edits or low-complexity tasks.
- Each retrospective must include at least one actionable improvement to prompts, workflow, or repo understanding.
- Base the retrospective only on changed files, commands, tests, audit artifacts, errors, corrections, repo structure, and explicit user instructions.

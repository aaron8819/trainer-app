...
function Get-DirtyPaths {
    ...
    $status = Invoke-GitRead -WorkingDirectory $WorktreePath -Arguments @('status', '--short', '--untracked-files=all') -AllowFailure
...
function Read-Worktrees {
    ...
    $lines = (Invoke-GitRead -WorkingDirectory $RepositoryRoot -Arguments @('worktree', 'list', '--porcelain')).Output
    ...
    $primaryPath = if ($records.Count -gt 0) { Get-NormalizedPath -Path $records[0].path } else { $null }
    ...
    foreach ($record in $records) {
        ...
        elseif ($normalized.StartsWith($canonicalNormalized + [System.IO.Path]::DirectorySeparatorChar, [System.StringComparison]::OrdinalIgnoreCase)) {
            $compliance = $true
        ...
        $result.Add([pscustomobject][ordered]@{
            path = $normalized
            branch = if ($record.PSObject.Properties.Name -contains 'branch') { $record.branch } else { $null }
            head = if ($record.PSObject.Properties.Name -contains 'head') { $record.head } else { $null }
...
try {
    ...
    if (Test-Path -LiteralPath $dependencyPath) {
        ...
        if ($dependencyItem.LinkType) {
            $dependencyState = $dependencyItem.LinkType.ToString().ToLowerInvariant()
            $dependencyTarget = @($dependencyItem.Target) -join '; '
        }
        else {
            $dependencyState = 'directory'
        }
    }
    else {

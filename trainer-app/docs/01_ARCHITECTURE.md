...
- Runtime exercise swaps now stay on that same ledger. The canonical write-side fact is `runtimeEditReconciliation.ops[].kind === "replace_exercise"` with the stable `workoutExerciseId` plus original/replacement exercise ids and names. The workout row mutates in place for current-session execution, while the receipt remains the original programmed truth and `workoutStructureState` marks the resulting drift.

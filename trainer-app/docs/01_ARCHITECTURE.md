...
- Weekly volume interpolation is centralized in `src/lib/engine/volume-targets.ts` and consumed by lifecycle math/engine volume services. The canonical target path is now `Mesocycle/Block context -> buildWeeklyVolumeTargetProfile() -> interpolateWeeklyVolumeTarget() -> getWeeklyVolumeTarget() -> generation/planning`, with duration-only interpolation retained only as a fallback when block coverage is missing or incomplete.
...
- `TrainingBlock` is now generation-relevant rather than explainability-only: `cycleContext.blockType` and `cycleContext.weekInBlock` come from the active block when available, with lifecycle fallback only for legacy/missing block data. The same block timeline now also shapes lifecycle weekly volume targets instead of only RIR/set progression.

...
- Typed pre-session readiness readout contracts are app-owned in `src/lib/api/pre-session-readiness-contract.ts`, with pure construction in `src/lib/api/pre-session-readiness-contract-builder.ts`. The workout-audit pre-session mode may produce that contract as an `audit_readout`, while the explicit app-owned producer in `src/lib/api/pre-session-readiness-producer.ts` prepares a `persisted_snapshot` contract, replaces older matching `PreSessionReadinessSnapshot` rows, and returns the same app-safe gym-card DTO. The persisted `PreSessionReadinessSnapshot` source is owned by `src/lib/api/pre-session-readiness-snapshot.ts` and validates through the same app-safe contract/DTO seam as a `persisted_snapshot`; Home must consume the app-safe contract and `src/lib/api/pre-session-readiness-gym-card.ts`, not audit-only payload fields, CLI prose, or artifact files.

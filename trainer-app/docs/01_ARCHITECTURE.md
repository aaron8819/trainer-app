...
- Session review distinguishes clean progression from recalibration: `src/lib/api/explainability.ts` may downgrade an engine load increase to `recalibrated_increase`, `recalibrate`, `target_too_high`, `insufficient_evidence`, or `caution_review_manually` when the performed anchor materially missed the written target, planned-set coverage was low/skipped, evidence was too thin, or transition-backfill provenance requires manual review. The engine trace remains preserved; user-facing copy must not present those cases as a plain increase.

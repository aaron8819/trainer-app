...
- Typed pre-session readiness readout contracts are app-owned in `src/lib/api/pre-session-readiness-contract.ts`. The workout-audit pre-session mode may produce that contract as an `audit_readout`, and a future persisted `PreSessionReadinessSnapshot` should use the same validator/DTO seam as a `persisted_snapshot`; Home must consume the app-safe contract and `src/lib/api/pre-session-readiness-gym-card.ts`, not audit-only payload fields, CLI prose, or artifact files.

...
- Generation/finalization build the canonical session decision under `selectionMetadata.sessionDecisionReceipt` in `src/lib/api/template-session.ts`, with planning-critical seams in `src/lib/planning/session-opportunities.ts`, `src/lib/api/template-session/role-budgeting.ts`, and `src/lib/api/template-session/closure-actions.ts`. When runtime placement already knows the advancing slot, that canonical slot context now enters generation before receipt finalization instead of being authored first in route-level post-processing. The same receipt also carries narrow session provenance (`sessionProvenance.mesocycleId` plus `sessionProvenance.compositionSource`) so generated sessions retain the active mesocycle and high-level composition boundary without copying audit-only execution-path metadata.
...
- Structural mutation reconciliation is persisted separately from the receipt under two additive selection-metadata fields owned by write-side mutation paths in `src/lib/api/runtime-edit-reconciliation.ts`:
  - `selectionMetadata.workoutStructureState`: current saved structure summary plus generated-vs-saved reconciliation
  - `selectionMetadata.runtimeEditReconciliation`: append-only v1 runtime edit facts plus conservative future-effect directives
- Runtime exercise swaps and removals stay on that same ledger. The canonical swap write-side fact is `runtimeEditReconciliation.ops[].kind === "replace_exercise"` with the stable `workoutExerciseId` plus original/replacement exercise ids and names. Runtime-added exercise removal appends `remove_exercise` after deleting the unlogged current-session row. The receipt remains the original programmed truth while `workoutStructureState` marks the current saved-workout structure.
- That seam is intentionally write-side only in v1. It does not own session semantics, progression eligibility, slot seeds, or read-model wording.
...
- Runtime readers in UI and explainability consume canonical selection metadata via `src/lib/ui/selection-metadata.ts` and `src/lib/ui/explainability.ts`: `sessionDecisionReceipt` remains original generated/evidence truth, `workoutStructureState` remains the canonical current-structure truth boundary, and `runtimeEditReconciliation` remains a persisted write-side ledger rather than a new read-model owner.
- Read-side swap wording for logging/review surfaces should derive from that `replace_exercise` op. Swapped exercises are replacements, not runtime-added extras, and future load/progression history stays exercise-specific to the replacement because the persisted `WorkoutExercise.exerciseId` now points at the executed substitute.
...
- Session review distinguishes clean progression from recalibration: `src/lib/api/explainability.ts` may reclassify a next-exposure decision to `recalibrated_increase`, `hold_at_recalibrated_anchor`, `recalibrate`, `target_too_high`, `insufficient_evidence`, or `caution_review_manually` when the performed anchor materially missed or materially exceeded the written target, planned-set coverage was low/skipped, evidence was too thin, or transition-backfill provenance requires manual review. Low-side misses imply the written target may be too high; high-side mismatches imply the written target may be too low. The engine trace remains preserved; user-facing copy must not present those cases as a plain clean increase or clean hold.
- The completed-workout review path is a shared read-side seam. Immediate completion review and `/workout/[id]` are separate presentation entries over the same `PostSessionReviewDisplayDto`, not separate progression or execution-summary interpreters.
- A `COMPLETED` workout finalizes one immutable `PostSessionReviewSnapshot` inside the workout-save transaction. Reopening uses that persisted contract as historical authority; `produceCurrentPostSessionReviewInterpretation()` is a separately named diagnostic/legacy compatibility seam and never overwrites a snapshot. `PARTIAL` remains resumable and therefore uses current interpretation until it reaches `COMPLETED`; `SKIPPED` has no post-session review.
- The normal post-workout UX is a presentation layer over that same immutable review data. The default completion/review path leads with one concise session result, next-time changes, material warnings, and compact weekly impact. Completion counts, calibration evidence, trends, and set-level results remain available behind disclosures, while `/workout/[id]/audit` remains the deeper verification surface.
...
- `src/lib/plan-types.ts` is the exhaustive application boundary for supported persisted plan types. `HYPERTROPHY` and `STRENGTH` are executable; unknown values fail closed rather than falling through to a default policy.
- `src/lib/api/plan-management.ts` owns owner-scoped plan listing, derived lifecycle status, plan-type dispatch for generated plan creation, explicit readiness finalization, optimistic rename, and soft archive. Routes and pages do not recompute READY state.
...
- Plan finalization changes only the generated plan's current-mesocycle readiness identity. Strength finalization also creates immutable accepted seed revision 1 from the generated executable seed in that same serializable transaction. It does not select the plan, rewrite an existing accepted revision, or mutate workout/receipt/history evidence. Archive is a soft `MacroCycle.archivedAt` transition and is forbidden for the selected plan.
...
- Plan-type lifecycle policy is exhaustive in `src/lib/plan-types.ts`. Hypertrophy retains the existing explicit successor-handoff flow. The first Strength increment is one complete four-to-six-week block, so finishing its deload closes that mesocycle and plan as `COMPLETED` instead of routing a Strength plan into the hypertrophy-specific successor authoring flow.
...
- Current-week projection treats a persisted incomplete workout as already-materialized execution state. `src/lib/api/persisted-incomplete-workout-projection.ts` reconstructs performed and remaining volume only from persisted workout exercises, sets, logs, frozen stimulus snapshots, the receipt, and the runtime-edit ledger. It does not regenerate that session or consult current catalog, seed, planner, or V2 diagnostics. Missing, corrupt, or structurally ambiguous evidence fails closed.
- `MesocycleExerciseRole` remains the data-layer continuity registry for intent-scoped role metadata and legacy carry-forward support, but it is no longer the seeded runtime composition owner. When an immutable revision exists, runtime exercise composition resolves exercise identity and set counts from `currentSeedRevision.seedPayload` plus slot-aware sequencing. `slotPlanSeedJson` is compatibility or historical state in that case; supported legacy/no-revision fallback does not create competing runtime truth.
...
- `findRelevantWeekCloseForUser()` remains the broad mesocycle-scoped selector for relevant week-close truth, including resolved-partial rows (`src/lib/api/mesocycle-week-close.ts`). Current-week operational surfaces must still apply an active/displayed-week guard before rendering that row. `loadHomeProgramSupport()` now only surfaces week-close support when the selected row matches both the active mesocycle and the active home week (`src/lib/api/program.ts`). `resolution` remains a compatibility field and must not be overread as deficit truth.
...
- `/program` page composition beyond that shared card now lives in `loadProgramPageData()` in `src/lib/api/program-page.ts`. That page-level read model owns the active-mesocycle overview, current-week slot-map presentation, compact week-completion outlook, lower volume-details composition, and advanced-actions availability for the Program page only. It must compose existing canonical seams rather than expanding `ProgramDashboardData` into a second all-purpose Program surface contract.
...
- `/` page composition beyond those operational helpers now lives in `loadHomePageData()` in `src/lib/api/home-page.ts`. That page-level read model owns Home-only continuity framing, recent-activity preview composition, and compact active-week context while delegating next-session and mesocycle truth to existing canonical seams instead of stitching raw workout Prisma reads inside `src/app/page.tsx`.
- Typed pre-session readiness readout contracts are app-owned in `src/lib/api/pre-session-readiness-contract.ts`, with pure construction in `src/lib/api/pre-session-readiness-contract-builder.ts`. The workout-audit pre-session mode may produce that contract as an `audit_readout`, while the explicit app-owned producer in `src/lib/api/pre-session-readiness-producer.ts` prepares a complete `persisted_snapshot` contract before entering the atomic persistence transition and returns the same app-safe gym-card DTO. `src/lib/api/pre-session-readiness-identity.ts` owns versioned exact identity and payload normalization/hashing; `src/lib/api/pre-session-readiness-snapshot.ts` owns in-transaction evidence revalidation, idempotent reuse, supersession, integrity validation, and exact-identity lookup, with PostgreSQL partial unique indexes as the final concurrency guard. Home and logging never select readiness by newest timestamp and must consume the shared app-safe contract/DTO seam, not audit-only payload fields, CLI prose, or artifact files.
- `loadNextWorkoutContext()` remains lifecycle-safe and checks pending mesocycle handoff first. Future rotation is derived from lifecycle and slot-runtime truth; an existing materialized incomplete workout instead preserves its persisted week/session snapshots and validates them against its canonical receipt slot/provenance plus the referenced immutable seed revision. Contradictory or insufficient materialized identity fails closed without generating a replacement. When a mesocycle is in `AWAITING_HANDOFF`, the canonical next-workout source is `handoff_pending` and generation/resume suggestions are intentionally suppressed until the next cycle is accepted. When final accumulation sessions are complete, target deficits alone must not route another accumulation session; only real lifecycle/data blockers such as linked pending closeout work can produce `final_week_close_pending` before deload routing.
- For active mesocycles, `loadNextWorkoutContext()` now resolves the next advancing session through the persisted runtime slot sequence when `slotSequenceJson` exists and returns canonical slot context for generation/audit reuse. `constraints.weeklySchedule minus performed intents` is retained only as a fallback for legacy mesocycles that do not yet have persisted slot identity.
...
- `WorkoutListSurfaceSummary` in `src/lib/ui/workout-list-items.ts` is the canonical workout/session summary read model for list surfaces. `/history`, `GET /api/workouts/history`, and the home-page recent-activity preview should anchor on this shape rather than ad hoc row contracts.
- Shared workout-list display semantics for those list surfaces now live with that contract in `src/lib/ui/workout-list-items.ts`: status labels/classes, intent labels, exercise/set count copy, explicit deload labeling, and optional-session labeling are centralized there so Home previews and History do not drift.

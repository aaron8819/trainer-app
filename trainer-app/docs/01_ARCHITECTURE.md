...
- Session review distinguishes clean progression from recalibration: `src/lib/api/explainability.ts` may reclassify an engine load increase to `recalibrated_increase`, `recalibrate`, `target_too_high`, `insufficient_evidence`, or `caution_review_manually` when the performed anchor materially missed or materially exceeded the written target, planned-set coverage was low/skipped, evidence was too thin, or transition-backfill provenance requires manual review. Low-side misses imply the written target may be too high; high-side mismatches imply the written target may be too low. The engine trace remains preserved; user-facing copy must not present those cases as a plain increase.

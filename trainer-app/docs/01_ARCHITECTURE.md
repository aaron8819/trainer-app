...
- Supplemental deficit sessions remain visible to history/volume/recovery read paths, but they are excluded from progression anchors and progression explainability history (`src/lib/progression/progression-eligibility.ts`, `src/lib/api/explainability.ts`).

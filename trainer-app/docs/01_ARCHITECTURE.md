...
- `loadNextWorkoutContext()` is receipt-agnostic and remains lifecycle-safe, but it now checks pending mesocycle handoff first. When a mesocycle is in `AWAITING_HANDOFF`, the canonical next-workout source is `handoff_pending` and generation/resume suggestions are intentionally suppressed until the next cycle is accepted. When final accumulation sessions are complete but the final accumulation week-close is still `PENDING_OPTIONAL_GAP_FILL`, the canonical next-workout source is `final_week_close_pending` and standard accumulation generation is blocked until the closeout is resolved or dismissed.

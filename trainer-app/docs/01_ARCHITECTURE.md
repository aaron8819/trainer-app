...
- `MesocycleExerciseRole` remains the data-layer continuity registry for intent-scoped role metadata and legacy carry-forward support, but it is no longer the seeded runtime composition owner. For accepted mesocycles with supported intents, runtime exercise composition now resolves exercise identity and set counts from `slotPlanSeedJson` plus slot-aware runtime sequencing.

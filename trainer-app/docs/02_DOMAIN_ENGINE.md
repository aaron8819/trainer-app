...
- Repeated-slot compound planning is now owned by `resolveSessionSlotProfile()` in `src/lib/planning/session-slot-profile.ts`. That pure seam resolves repeated-slot identity from canonical runtime slot sequencing and provides the small compound-bias profile used upstream by `buildSelectionObjective()` and `selection-v2` before closure/diversification.

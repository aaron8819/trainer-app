...
- Repeated-slot planning policy is now owned by `resolveSessionSlotPolicy()` in `src/lib/planning/session-slot-profile.ts`. That pure seam resolves canonical current-session and future-slot policy from runtime slot sequencing, supplies the current-session `compoundBias` consumed upstream by `buildSelectionObjective()` and `selection-v2`, and exposes the intentionally minimal future-slot bias used by remaining-week planning before closure/diversification.

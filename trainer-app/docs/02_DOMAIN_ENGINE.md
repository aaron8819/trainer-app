...
- Program-quality constraints are a projection-only shaping layer in `src/lib/api/mesocycle-handoff-slot-plan-projection.program-quality.ts`, applied after slot-local exercise selection and again after obligation/support repair before seed serialization. The layer uses additive monotonic soft penalties with fixed priority order: P0 weekly obligations/slot identity, P1 weekly movement-pattern balance, P2 per-exercise efficiency/session main-compound count, P3 stimulus diversity and spread, P4 duplicate/redundancy penalties, and P5 deficit-driven isolation completeness. It may move sets across already-selected compatible exercises, demote excess main compounds, and add direct Biceps/Triceps/Side Delts isolation only when projected support remains below the existing Week 1 support floor and slot identity has room. It must not change `slotPlanSeedJson` schema, runtime seeded replay, route behavior, receipts, or persistence semantics.
...
- Secondary/low-priority readout targets are explicit display semantics in `src/lib/engine/volume-landmarks.ts`, not lifecycle MEV changes. `getMuscleTargetSemantics()` classifies Core, Forearms, Adductors, Abductors, and Lower Back as `soft` targets with display ranges while preserving their existing `VOLUME_LANDMARKS.mev=0`; generation, week-close deficits, protected-floor repair, Accessory Lane, and accept gating continue to consume hard lifecycle targets from `getWeeklyVolumeTarget()`.

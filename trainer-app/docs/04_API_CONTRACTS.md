...
- Exercises and preferences: `src/app/api/exercises/**`, `src/app/api/preferences/route.ts`
  ...
  - `GET /api/exercises/[id]/history?limit=<n>` returns exact-exercise performed history owned by `loadExerciseHistory()`. The workout logger additionally supplies `measurementSnapshot=legacy` or a complete classified profile/convention/basis query so the active workout snapshot—not the newest historical row—selects the cohort; malformed partial tuples return `400`. Library-only reads use the newest compatible cohort. Only performed work-set logs from performance-history-eligible `COMPLETED`/`PARTIAL` workouts qualify; warmups, skipped/unlogged sets, scheduled deloads, prescriptions, aliases, and related exercises do not. Classified cohorts require an exact frozen measurement tuple match and never mix with legacy rows. Bodyweight, displayed-machine, and displayed-assistance cohorts remain visible but do not produce computed load records or claim physical units because bodyweight or stable machine context is not persisted.
...
- Prescription-calibration rows classify `successful_autoregulation` when performed load differs but prescribed reps are achieved at or below target RPE; load delta alone is insufficient for `target_too_high` or `target_too_low`. Compact exact-exercise trend groups describe a supported mismatch as systematic only when it appears in at least two of the latest three comparable exposures.
...
- Strength creation produces one complete four-to-six-week block according to saved training age. After its final deload session, shared lifecycle state closes that plan as `COMPLETED`; the hypertrophy-only next-mesocycle handoff endpoints are not offered for it. Terminal lifecycle selection requires an explicit supported `MacroCycle.primaryGoal`: only `HYPERTROPHY` enters handoff and only `STRENGTH` completes terminally. Missing, null, malformed, or future values throw `UNSUPPORTED_PLAN_TYPE`, and transaction callers roll back without partial workout or lifecycle mutation.
...
- `POST /api/mesocycles/[id]/accept-next-cycle` (`src/app/api/mesocycles/[id]/accept-next-cycle/route.ts`)
  ...
  - `currentSeedRevision.seedPayload` is the authoritative accepted executable composition when an immutable revision exists. The accepted-seed dispatcher validates V1, V2, or V3 before normalization, review, editable copy, activation/materialization, or measurement-aware runtime decisions; unsupported versions and malformed versioned payloads fail explicitly instead of falling through an older schema. V1 and V2 retain their existing canonical normalization and hashing. Custom Hypertrophy V3 preserves the editable V2 envelope and adds a hash-covered measurement snapshot to every exercise; runtime consumes its source-neutral executable V2 projection containing only `exerciseId`, `role`, `setCount`, and `measurement`. `slotPlanSeedJson` remains the compatibility or historical acceptance snapshot. Legacy identity-only/no-revision seeds and optional `acceptedPlannerIntent` remain readable through compatibility parsing, but that fallback does not create competing runtime truth and planner intent is explanatory rather than executable revision truth.
...
- `POST /api/plans`: custom hypertrophy creation accepts name, 2–6 sessions, equipment, duration, author method, an optional manual preset, and an optional client-generated `creationId`. The UI retains one `creationId` for retries of the same logical payload and generates a new one when the payload changes. The server uses that UUID as the existing unique plan identity, deterministically derives new draft slot identities from it, replays only an owner/name/payload-equivalent duplicate, and returns `409 code=PLAN_CREATION_ID_CONFLICT` for conflicting reuse. No time, title, or content-window heuristic is used. V2 generation requires four sessions. `authorMethod="WEEKLY"` creates a strict Draft V2 with generic session structure, four accumulation weeks, and a final deload; it does not create a mesocycle or accepted programming.
- `PATCH /api/plans/[id]/draft`: atomically saves normalized plan name plus the complete strict Draft V1 or Draft V2 using `expectedRevision`; stale writes return the existing plan mutation conflict response. A structurally valid but overall-incomplete Draft V2 can save. Malformed placements fail validation before the CAS write. A successful Draft V2 response includes a non-persisted `preview` union: deterministic ineligibility reasons, or normalized accepted/executable payloads plus SHA-256 hash when eligible.
- `POST /api/plans/[id]/regenerate`: replaces a four-session Draft V1 through V2 only when `replaceConfirmed=true` and the expected revision still matches. Draft V2 returns `409 code=PLAN_VERSION_NOT_EXECUTABLE` before replacement.
- `POST /api/plans/[id]/finalize`: Draft V1 custom requests supply `expectedDraftRevision` and `warningsConfirmed`. Make-ready validates health inside a serializable transaction, requires warning confirmation when applicable, writes accepted truth/projections, and consumes the draft. Any request-body shape targeting a persisted Draft V2 returns the exact `409 code=PLAN_VERSION_NOT_EXECUTABLE` contract before owner provisioning or lifecycle mutation; the generic finalization owner repeats the persisted-version guard inside its transaction before timestamp or lifecycle handling.
- `POST /api/plans/[id]/copy`: creates a new draft from a source plan's current version 2 or version 3 accepted revision, preserving settings, slot names/focus, exercise intent, and bounded `requiredExerciseClass`. It never infers semantic intent from exercise identity. Legacy plans without preserved intent are not copyable through this contract.

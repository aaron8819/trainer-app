...
- Saved placement correlations are serialized evidence, not trusted read authority. `src/lib/session-semantics/placement-correlation.ts` validates generated sources, persisted targets, and one-to-one cardinality before any placement-sensitive read uses them. Explicit missing targets, duplicate sources, duplicate targets/many-to-one mappings, unknown sources, malformed records, and duplicate-identical records fail closed; an invalid explicit source cannot fall through to canonical exercise-ID matching. Unique canonical fallback is reserved for genuinely absent legacy correlation, and placement-sensitive readiness/log guidance, explainability, audit/weekly review, and deload reads consume only resolver-proven pairs.

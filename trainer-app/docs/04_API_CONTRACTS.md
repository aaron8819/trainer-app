...
- Structural mutation contract:
  ...
  - `GET /api/workouts/[id]/swap-exercise?workoutExerciseId=<id>&q=<query>&limit=<n>` returns a bounded typed-search shortlist narrowed within the same server-enforced eligible replacement universe; text relevance bounds the search set, then the final candidate list is re-ranked by canonical runtime swap eligibility and read-only lane-fit diagnostics before it reaches the client

...
- `POST /api/plans/[id]/finalize`: custom requests supply `expectedDraftRevision`, `warningsConfirmed`, and for Draft V2 the saved preview's `confirmedPreviewHash`. Make-ready re-reads and revalidates inside a serializable transaction, requires warning confirmation, rejects stale hash with `PLAN_PREVIEW_HASH_MISMATCH`, rejects non-supported weekly topology with `PLAN_UNSUPPORTED_TOPOLOGY`, creates immutable accepted revision 1, and consumes the draft last. Rollout-disabled custom-shaped requests fail explicitly before owner provisioning. Finalization does not activate the plan.

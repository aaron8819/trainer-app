...
- Lifecycle completion counters (`completedSessions`, `accumulationSessionsCompleted`, `deloadSessionsCompleted`) are incremented only on the first transition to `COMPLETED`, after the workout compare-and-swap and inside the same save-workout transaction (`src/app/api/workouts/save/route.ts`). `PARTIAL` remains performed history but is not a completed-session counter event.
...
- Compatible non-V4 lifecycle dispatch uses the frozen `slotSequenceJson` week-by-slot obligations after either a newly committed `COMPLETED` or `SKIPPED` result. Both statuses resolve their authored obligation, but only `COMPLETED` increments `completedSessions` and the phase completion counter. `PARTIAL` does neither. Explicit non-advancing sessions and out-of-schedule rows are excluded; ambiguous in-schedule claims fail closed. Duration-aware counter thresholds remain only as compatibility behavior when no frozen legacy topology exists.

...
- Accepted V4 is the narrow exception: only a newly committed `COMPLETED` increments its phase counter. `PARTIAL` remains performed but schedule-unresolved; `SKIPPED` is schedule-resolved without a counter increment. The route rereads authoritative workout evidence, resolves all frozen required slots, and advances phase or enters shared handoff only from that exact result. Counter thresholds do not close V4 independently.
...
- For accepted V4 advancing sessions, save adds a server-authored immutable `scheduledSlotReceipt` under that canonical receipt. It binds mesocycle, current accepted revision id/number/hash, week, stable slot id, frozen index, and sequence length to `Mesocycle.slotSequenceJson`; conflicting client metadata, stale/malformed identity, or duplicate claims return recoverable `409` before lifecycle advancement.

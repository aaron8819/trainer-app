...
- `POST /api/logs/set` accepts optional `setIntent: "WORK" | "WARMUP"` and defaults omitted values to `WORK`. `WARMUP` is explicit user capture for warmup/ramp performed reality; it does not infer from load, order, reps, or RPE, and it does not mutate receipts, seed/runtime replay, planner metadata, or workout structure.

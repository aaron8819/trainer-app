...
- `POST /api/plans/[id]/copy` reads only the source mesocycle's exact current accepted revision. It supports the single executable active mesocycle or the deterministic latest completed mesocycle, rejects multiple active rows or duplicate latest mesocycle numbers as ambiguous, validates revision ownership/pointer/provenance/hash before copying, and never falls back to compatibility seed JSON.

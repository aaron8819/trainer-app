...
- `POST /api/mesocycles/[id]/accept-next-cycle` (`src/app/api/mesocycles/[id]/accept-next-cycle/route.ts`)
  ...
  - `slotPlanSeedJson` stores ordered `slotId -> exercises[{ exerciseId, role, setCount }]` data from the final canonical handoff candidate, remains distinct from setup display DTOs, and becomes the canonical seeded runtime composition source for supported accepted mesocycles. The contract may also carry optional seed-safe `acceptedPlannerIntent` metadata, including runtime-inert Stage 1 capacity explanation; runtime replay ignores it. For newly accepted seeded mesocycles, `setCount` is required and runtime replays it as an explicit override rather than recomputing sets. Legacy identity-only seeds and accepted-intent payloads without capacity metadata remain readable through compatibility parsing.

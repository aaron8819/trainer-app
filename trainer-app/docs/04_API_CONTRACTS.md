...
- Generation responses return canonical selection metadata and server-owned prescription readouts only:
  ...
  - the readout preserves final generated reps/range/RPE, derives representative `targetRir` as `10 - targetRpe`, maps load kind/source/confidence from the canonical prescription result, and copies measurement profile/load convention/rep basis/zero meaning from the final frozen workout exercise. Numeric zero, semantic zero, calibration required, not applicable, and unavailable/null remain distinct. Missing or mismatched placement results fail closed to an unavailable row and never borrow another occurrence's result.

...
- `ProgramDashboardData.volumeThisWeek` rows also expose UI-owned volume status display as `statusLabel`, `statusDescription`, and `badges`. Client components render these server-shaped labels instead of classifying MEV/MAV/MRV ranges locally.

...
- Generation responses return canonical selection metadata and server-owned prescription readouts only:
  ...
  - both routes may return optional `prescriptionReadouts` (`PrescriptionReadout[]` from `src/lib/api/template-session/types.ts`). Each row is keyed by final `WorkoutExercise.id` as `placementId`, so repeated occurrences of the same canonical `exerciseId` remain distinct. The route assembles the projection only after readiness/autoregulation, gap-fill caps, and Short Today/session-capacity reduction, using the final returned workout plus the exact placement-keyed final `PrescriptionResult`; removed exercises cannot leave rows and `setCount` is the retained working-set count.
  - the readout preserves final generated reps/range/RPE. Representative `targetReps`, `repRange`, and `targetRpe` come from final working set index `1` when present (otherwise the first retained working set); representative `targetRir` is `10 - targetRpe`, not a separate set-level prescription. It maps load kind/source from the canonical result and copies measurement profile/load convention/rep basis/zero meaning from the final frozen workout exercise.
  - `confidence` is nullable. Numeric engine confidence maps `high -> high`, `reduced -> medium`, and `low -> low`; calibration-required maps its canonical `low`; semantic-zero, not-applicable, and unavailable return `null` because those engine result kinds carry no confidence assertion. Readiness consumers treat `null` as not applicable and do not turn it into a warning.
  - `cautionLevel` is a bounded display category derived only from canonical kind/confidence: calibration-required and unavailable are `caution`, reduced numeric is `notice`, low numeric is `caution`, and high numeric/semantic-zero/not-applicable are `none`. `cautionReason` uses stable state-derived readout codes rather than selecting a reason by array position. The projection does not alter load, reps, generation, readiness, Save, or materialization.
  - every final non-warmup placement requires an exact placement-keyed `PrescriptionResult`. Projection validates canonical exercise ID, the duplicated frozen measurement tuple (`profile`, `loadConvention` where applicable, and `repBasis`), and semantic-zero meaning. Missing or mismatched evidence throws `PrescriptionReadoutProjectionError`; generation routes return a generic internal `500` response and do not expose a normal authoritative readout or raw invariant details. A valid engine-authored `kind="unavailable"` still projects normally with null target/source/confidence, so canonical unavailability remains distinct from projection corruption.
  - `prescriptionReadouts` is additive, optional, display-safe response metadata. It may describe accepted scheduled workouts, explicit previews, and recognized non-scheduled workouts, but contains no eligibility, authored slot/revision, materialization, receipt, or Save fields. Preview readouts do not weaken `preview_only`; non-scheduled readouts make no authored claim; accepted-V4 template materialization still rejects before projection. The array is not persisted in accepted seeds or receipts and is not consumed as Save authority.
...
- Template replacement requests identify the exact occurrence with `exerciseReplacements[*].placementId` (the stable template-exercise row identity). `orderIndex` and `originalExerciseId` must still match that placement; neither canonical exercise identity nor first-match lookup is sufficient. Replacement responses preserve the placement ID while regenerating canonical measurement, prescription, readiness, audit, and readout state.
- Workout save exercise rows may carry the generated `placementId`. Save allocates the persisted workout-exercise row IDs and stores an explicit generated-placement-to-persisted-row correlation in the session-audit saved layer. No Prisma/schema change is involved.
- Saved placement correlations are serialized evidence, not trusted read authority. `src/lib/session-semantics/placement-correlation.ts` validates unique generated/persisted occurrence IDs from the original input arrays before constructing ID maps, then validates generated sources, persisted targets, and one-to-one explicit cardinality before any placement-sensitive read uses them. Explicit missing targets, duplicate sources, duplicate targets/many-to-one mappings, malformed records, duplicate-identical records, and duplicate input occurrence IDs fail closed. A source-less malformed record blocks legacy fallback for the resolver scope; an invalid named source cannot steal a valid target or authorize fallback for a named generated occurrence. Unique canonical fallback is reserved for a resolver-proven genuinely absent legacy remainder, and placement-sensitive readiness/log guidance, explainability, audit/weekly review, and deload reads consume only resolver-proven pairs. Log guidance exposes only a persisted-placement map; it has no exercise-ID or exercise-name attachment path.
...
- For exact accepted V4 schedules, both generation routes additionally pass the complete server-resolved `V4ScheduledGenerationObligation` (`V4ScheduleAuthority` plus `V4RequiredSlot`) into generation. Context loading re-resolves the current accepted revision and fails closed if the authority or required slot changed. The authored obligation's `weekInMeso` drives projected-week recommendations, periodization, accepted-seed replay, and receipt `cycleContext`; completed-session counters cannot reconstruct or override that identity. Save still independently re-resolves the schedule, adds the immutable `scheduledSlotReceipt`, and rejects stale, conflicting, or duplicate claims.
- Lower generation APIs require an explicit schedule mode. `accepted_v4_scheduled` requires the canonical obligation and a one-use in-process capability issued only after current persisted-claim revalidation; raw mapped-context and raw deload composition reject scheduled mode. Capability consumption checks the complete mesocycle/revision/hash/week/phase/slot/intent/sequence tuple before composition, so omission or a structurally assembled lookalike cannot enter a counter-derived fallback. `explicit_preview` carries a caller-explicit authored week/slot and does not claim current eligibility. `non_scheduled` permits recognized body-part, supplemental, gap-fill, and closeout work alongside an accepted V4 plan without emitting a required-slot claim or advancing the authored schedule. Only `legacy` may retain lifecycle-derived week behavior, and only outside accepted-V4 scheduled ownership.
- Every generated decision receipt carries explicit server-authored materialization evidence. `accepted_v4_scheduled -> scheduled_required`, `explicit_preview -> preview_only`, recognized `non_scheduled -> non_scheduled + purpose`, and historical missing evidence normalizes to `legacy`. Save rejects `preview_only`, rejects incoherent or changed classification, validates each non-scheduled purpose against the shared gap-fill/supplemental/body-part/closeout classifiers, and still independently re-resolves current accepted-V4 authority and persisted claims for `scheduled_required`. Classification describes what the artifact may become; it is never schedule authority. Closeout canonicalization is server-owned and rewrites the receipt to `non_scheduled/closeout` only through the existing validated closeout flow.
- Accepted-V4 scheduled template materialization is intentionally prohibited. `POST /api/workouts/generate-from-template` returns `409` with `code="V4_SCHEDULED_TEMPLATE_MATERIALIZATION_UNSUPPORTED"` when the request resolves an accepted-V4 scheduled obligation; it does not emit a runtime-selection receipt that Save would reject. Accepted-V4 scheduled workouts must use the canonical intent-generation path and accepted-seed replay. Save provenance requirements are unchanged.
- Pre-session readiness must resolve the exact current obligation and forwards `accepted_v4_scheduled`; an accepted-V4 identity gap returns an integrity blocker. Logging weekly-volume guidance reconstructs the materialized workout's accepted revision and authored slot, excludes `COMPLETED`/`SKIPPED` claims, and projects only unresolved obligations from that same authored week without invoking legacy slot rotation. Each projected obligation receives a distinct `explicit_preview` mapped context bound to its own authored slot before seed lookup, prescription composition, and receipt finalization; mapped contexts are not reused across obligations, including deload slots with repeated intents. Projected-week volume throws the typed `V4_PROJECTED_SCHEDULE_UNAVAILABLE` read error for blocked/ambiguous/missing accepted-V4 authority rather than substituting the lifecycle week. Workout-audit next-session generation follows the scheduled-obligation rule only when it is materializing the actual next session. Post-accept verification and arbitrary accepted-plan inspection instead pass the authored week/slot explicitly as `explicit_preview`, receive `preview_only`, never acquire scheduled composition capability, and remain unsaveable.

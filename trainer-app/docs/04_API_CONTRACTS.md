...
- `POST /api/plans` is a discriminated contract. `{ planType: "HYPERTROPHY", name, startDate, durationWeeks }` preserves existing generation. `{ planType: "STRENGTH", name, startDate, configuration }` requires emphasis, two-to-five training days, a hard 45/60/75/90-minute session budget, equipment profile, and squat/press/hinge preferences; saved training age and active injury limitations are loaded server-side. Common low/lower-back, knee, shoulder, hip, elbow, and wrist phrasing is canonicalized server-side. Unclassifiable active anatomy returns `409 code=PLAN_LIMITATION_UNRECOGNIZED` before any plan row is created. A missing `planType` remains backward-compatible hypertrophy input. Unknown types, partial configuration, and extra fields fail validation. Creation persists `PREPARING` and never changes `User.activeMacroCycleId`.
...
- `POST /api/mesocycles/[id]/accept-next-cycle` (`src/app/api/mesocycles/[id]/accept-next-cycle/route.ts`)
  ...
  - `currentSeedRevision.seedPayload` is the authoritative accepted executable composition when an immutable revision exists. It stores ordered `slotId -> exercises[{ exerciseId, role, setCount }]` data normalized from the accepted handoff candidate; runtime replays `setCount` as an explicit override rather than recomputing it. `slotPlanSeedJson` remains the compatibility or historical acceptance snapshot in that case. Legacy identity-only/no-revision seeds and optional `acceptedPlannerIntent` remain readable through compatibility parsing, but that fallback does not create competing runtime truth and planner intent is explanatory rather than executable revision truth.

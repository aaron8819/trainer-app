...
- On `/program`, `ProgramStatusCard` renders the Volume Details summary badges from the currently viewed `volumeThisWeek[*].badges` only; it no longer emits zero-count static categories, re-buckets target deltas, or recomputes weekly status locally.

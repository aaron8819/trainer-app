...
- Exercise queue rows render compact target-muscle badges from the server-shaped `LogExerciseInput.muscleTags` read model. That display is derived from canonical exercise muscle metadata plus the shared stimulus helper (`src/lib/ui/exercise-muscle-tags.ts`, `src/lib/ui/workout-sections.ts`) rather than component-level name heuristics.

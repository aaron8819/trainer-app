...
- `/program` also renders a compact forward-looking `weekCompletionOutlook` from the same page read model. Projection math remains owned by `loadProjectedWeekVolumeReport()` (`src/lib/api/projected-week-volume.ts`), while the Program page adapter narrows that audit-shaped output into assumption copy, server-owned status counts, filter badges, full display rows, and a compact default list using `classifyMuscleOutcome()` (`src/lib/api/muscle-outcome-review.ts`) plus shared weekly-muscle status labels. Hard targets are grouped as the primary projected landing summary; soft readout targets such as Core, Forearms, Adductors, Abductors, and Lower Back render separately with below/within/above soft-range language and do not inflate hard-target warning buckets. The page then applies client-only badge filtering over the preclassified primary payload without formatting, refetching, or re-bucketing outcomes locally.
...
- `ProgramStatusCard` renders weighted weekly volume from server-provided display strings (`weightedSetsLabel`, `targetLabel`, `deltaLabel`, optional `landmarkContext`, `statusLabel`, `statusDescription`, and `badges`) and treats raw `directSets` / `indirectSets` as contextual structural copy only (`Raw sets: 13 direct, 4 indirect`). It splits rows by server-provided `displayGroup`: primary hypertrophy targets render first and own the hard-target summary chips, while secondary soft targets render below with `Soft target: N-M weighted sets`, soft-range status labels, and non-blocking copy. The component does not format or classify MEV/MAV/MRV thresholds or soft ranges locally (`src/lib/api/program.ts`, `src/components/ProgramStatusCard.tsx`, `src/components/ProgramStatusCard.render.test.tsx`).

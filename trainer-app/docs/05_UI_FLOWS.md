...
- Current-week slot rows carry server-shaped planned exercises plus a compact per-slot projected contribution. The exercise list is page-owned in `loadProgramPageData()` and prefers accepted `slotPlanSeedJson` slot order, exercise names, and canonical `setCount` when resolvable; planned workout `WorkoutSet` counts and projected remaining-session exercises from `loadProjectedWeekVolumeReport()` are fallback sources only when the seed cannot safely answer. Completed slots still show seed `setCount`, not drifted runtime counts. The Program page renders `Exercises unavailable` rather than reconstructing plans client-side. The contribution hint is derived only by matching each row's canonical `slotId` to `loadProjectedWeekVolumeReport().projectedSessions[*].slotId`, then reading that session's `projectedContributionByMuscle`. The shaped summary shows the top three positive contributors with `+N more` for hidden contributors. Missing matches render no contribution rather than borrowing another slot's projection.

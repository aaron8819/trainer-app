...
- Current-week slot rows can also carry a compact per-slot `impact` hint. That hint is page-owned in `loadProgramPageData()` and is derived only by matching each row's canonical `slotId` to `loadProjectedWeekVolumeReport().projectedSessions[*].slotId`, then reading that session's `projectedContributionByMuscle`. Missing matches render no impact rather than borrowing another slot's projection.

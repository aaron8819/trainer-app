...
- Swap preview is server-owned even before the UI consumes it. `GET /api/workouts/[id]/swap-exercise-preview` resolves the exact post-swap exercise payload from the same `src/lib/api` seam used by `POST /api/workouts/[id]/swap-exercise`, so preview and commit cannot diverge on reps, load hint, target RPE, rest, or id continuity (`src/app/api/workouts/[id]/swap-exercise-preview/route.ts`, `src/lib/api/runtime-exercise-swap-service.ts`).

...
- Add Exercise sheet mobile polish is owned locally by `src/components/BonusExerciseSheet.tsx`: the search field should use mobile-safe input sizing and search-oriented keyboard hints, add buttons should meet the same 44px-class tap target bar as the rest of the log UI, and each reopen should intentionally reset local query/error state while leaving server preview/add semantics unchanged.

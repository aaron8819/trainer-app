...
- Current-week slot rows can also carry a compact per-slot `impact` hint. That hint is page-owned in `loadProgramPageData()` and is derived only by matching each row's canonical `slotId` to `loadProjectedWeekVolumeReport().projectedSessions[*].slotId`, then reading that session's `projectedContributionByMuscle`. The shaped summary keeps the leading contributors compact, preserves ties and near-ties at the visible cutoff, and uses `+N more` for hidden positive contributors. Missing matches render no impact rather than borrowing another slot's projection.

...
- Runtime-added exercises with zero logged sets show a `Remove` action in the log queue. The client calls `DELETE /api/workouts/[id]/exercises/[exerciseId]`, then removes the row from local queue state, clears any removed-set draft/active references, and lets remaining-set counts recompute from the updated queue. Planned exercises and runtime-added exercises with any logged set do not expose removal (`src/components/LogWorkoutClient.tsx`, `src/components/log-workout/WorkoutExerciseQueue.tsx`, `src/components/log-workout/useWorkoutSessionFlow.ts`, `src/lib/api/runtime-exercise-remove-service.ts`).
...
- `ProgramStatusCard` separates weekly volume status from live current-week opportunity copy. Each card shows a primary weekly-status chip derived from weighted volume position (`Below MEV`, `In range`, `Near target`, `On target`, `Near MRV`, `At MRV`) plus a lighter current-week-only `Today:` advisory derived from `opportunityState` (`Today: room for more`, `Today: optional`, `Today: covered`, `Today: go lighter`). `Below MEV` uses neutral slate presentation, productive statuses (`In range`, `Near target`, `On target`) use an increasingly strong green-family presentation, and `Near MRV` / `At MRV` remain caution/risk states. Historical week views still hide the live-only `Today:` advisory because the current server model uses present recency/readiness context rather than historical replay.
- On `/program`, `ProgramStatusCard` renders the Volume Details summary badges from the same currently viewed `volumeThisWeek` rows as the grid, using the shared weekly-status ladder (`src/lib/ui/weekly-muscle-status.ts`); that section no longer mixes in the separate target-delta outcome-review classifier or a static active-week summary.
...
- Home active-week optional completion is composed through `OptionalWeekCompletion`, which combines active-week optional gap-fill and active-week custom-session presentation without changing either workflow. Prior-week optional-session availability remains outside that active-week module so it is not mixed with current-week gap-fill context (`src/components/OptionalWeekCompletion.tsx`, `src/lib/api/program.ts`, `src/lib/api/home-page.ts`, `src/app/page.tsx`).

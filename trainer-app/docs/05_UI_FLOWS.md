...
The weekly editor configures 1–52 contiguous weeks subject to reserving the last week for an optional deload, then edits sets, exact/range reps, RIR range/non-applicability, and deload-only omission for each stable exercise placement. Numeric text remains temporary UI state until it forms a valid placement prescription; while malformed, autosave pauses and shows `Incomplete — not saved`, preserving the last saved draft. Prescription inputs are keyed by placement, week, and phase: unchanged semantic weeks retain local malformed text, while accumulation/deload transitions remount from the migrated canonical prescription. Reorder and exercise replacement retain `placementId`; adding a placement creates a new UUID. Back cancels pending debounce, serializes any in-flight save with the exact latest valid draft, and navigates only after the authoritative response; failure offers retry-and-leave, stay, or explicit discard, while malformed input can only be corrected or explicitly discarded. The preview panel is unavailable while local changes are malformed or unsaved, then shows server-derived deterministic blockers or the normalized saved weekly plan and hash. No make-ready or activation action is rendered for Draft V2.

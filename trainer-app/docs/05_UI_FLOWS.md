...
- `ProgramStatusCard` renders weighted weekly volume from server-provided display strings (`weightedSetsLabel`, `targetLabel`, `deltaLabel`, optional `landmarkContext`, `statusLabel`, `statusDescription`, and `badges`) and treats raw `directSets` / `indirectSets` as contextual structural copy only (`Raw sets: 13 direct, 4 indirect`). The component does not format or classify MEV/MAV/MRV thresholds locally (`src/lib/api/program.ts`, `src/components/ProgramStatusCard.tsx`, `src/components/ProgramStatusCard.render.test.tsx`).

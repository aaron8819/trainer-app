...
- `/` now composes its page-local presentation through `loadHomePageData()` (`src/lib/api/home-page.ts`): one primary decision module with concise server-owned reason labels, a server-owned `primaryAction`, a concise continuity section (`last completed -> next due`) with slot-aware identity descriptors when canonical data exists, compact active-week context, optional operational exception cards, the shared compact `ProgramStatusCard`, and a 3-row recent-activity preview. Active-week progress is shaped from the actual current-week performed advancing-session count supplied by `loadHomeProgramSupport()` (`src/lib/api/program.ts`), while the next-due label remains delegated to canonical next-session resolution. Required generation is represented as `primaryAction.state="planned"` with `mode="generate"` rather than a separate UI state.

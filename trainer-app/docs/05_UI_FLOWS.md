...
- On `/program`, the compact `VolumeSnapshotSection` renders the default Weekly Volume Snapshot from the active `volumeThisWeek` rows already loaded for the Program page. It summarizes primary/support counts, shows only primary/support rows that need attention or high-watch review, hides on-target and secondary detail by default, and links to `/analytics` for deeper volume review without recomputing weekly status locally.

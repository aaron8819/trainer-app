...
- Current-week closeout visibility on `/program` is separate from that slot map. When a current-week closeout exists, the page shows a dedicated `Closeout` section/status row without turning that workout into an extra slot or projected remaining canonical session (`src/lib/api/program-page.ts`, `src/app/program/page.tsx`).
...
- Home closeout visibility is separate from canonical next-session guidance. When a current active-week closeout exists, Home renders a distinct `Closeout` card with its own status/action while leaving the main next-session card and continuity reasoning unchanged (`src/lib/api/program.ts`, `src/lib/api/home-page.ts`, `src/app/page.tsx`).

...
- Post-workout review should prioritize severe prescription-quality misses over routine progression wins. A major target-too-high or target-too-low recalibration should surface even when the base action is hold, because the most useful coaching message may be "the written target was wrong," not "hold/increase."
...
- On `/program`, the compact `VolumeSnapshotSection` renders the default Weekly Volume Snapshot from the active `volumeThisWeek` rows already loaded for the Program page. It summarizes primary/support counts, shows only primary/support rows on the watch list or high-watch review, hides on-target and secondary detail by default, and links to `/analytics` for deeper volume review without recomputing weekly status locally.
...
- `ProgramStatusCard` remains the shared program/volume card for Home (`src/app/page.tsx`) and the reusable detailed card surface (`src/components/ProgramStatusCard.tsx`). `/program` no longer mounts the full `variant="volumeOnly"` card by default; its lower weekly-volume area is page-owned by `VolumeSnapshotSection` so the Program view stays focused on decision support instead of full analytics.

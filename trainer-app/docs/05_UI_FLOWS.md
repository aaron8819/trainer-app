...
- Active-week custom-session visibility on `/program` is rendered through the same `OptionalWeekCompletion` presentation module as Home, separate from the ordered slot map. Previous-week closeout cards remain outside that active-week module and attached to their original week (`src/components/OptionalWeekCompletion.tsx`, `src/lib/api/program-page.ts`, `src/app/program/page.tsx`).
- Previous-week closeout cards on `/program` can still expose `Skip closeout`, which marks only the closeout dismissal metadata and then hides the card from the page read model without advancing slot sequencing or satisfying week-close deficit logic. The active-week optional module's hide action is UI-only.
...
- Home active-week optional completion is composed through `OptionalWeekCompletion`, which combines active-week optional gap-fill and active-week custom-session presentation without changing either workflow. Prior-week closeout availability remains outside that active-week module so it is not mixed with current-week gap-fill context (`src/components/OptionalWeekCompletion.tsx`, `src/lib/api/program.ts`, `src/lib/api/home-page.ts`, `src/app/page.tsx`).
- The unified active-week module has UI-only hide/show state scoped to the active week. It does not call closeout dismissal, week-close dismissal, or mutate deficits/lifecycle state.

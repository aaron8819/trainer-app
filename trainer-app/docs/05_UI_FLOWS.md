...
- `/workout/[id]` stays compact, but now leads with a post-workout hierarchy for performed sessions: `Session outcome` -> `Key lift takeaways` -> `Program impact` -> detailed set review. This is the main user path and should answer "how it went / what it means / what happens next" before raw set-level trace.
- Immediate completed-workout review follows the same hierarchy after save, then links into the full workout review page for the same session-level read plus the detailed set log.

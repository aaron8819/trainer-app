...
- Playwright config: `playwright.config.ts`; by default the database-scrubbing runner starts a managed local Next dev server on port `3217` with `UI_AUDIT_FIXTURE_MODE=1`, probes the dedicated `/ui-audit-fixture/ready` route, uses the isolated `.next-ui-audit/managed` output directory, and runs the core-route audit at mobile (`390x844`) and desktop (`1366x768`) viewport sizes.
- The UI audit fixture harness is development-only. `scripts/run-ui-audit.mjs` removes `DATABASE_URL` and `DIRECT_URL` from the Playwright and managed-server environments. Fixture access requires `UI_AUDIT_FIXTURE_MODE=1`, non-production `NODE_ENV`, and a recognized `x-ui-audit-fixture` request header. The proxy redirects gated page requests to the dedicated fixture route and rejects unhandled gated API requests before production route modules can load. Query parameters alone cannot enable fixture behavior, and production mode remains inert.
...
- Static boundary tests recursively inspect the fixture runtime import graph for database/Prisma imports and assert that the audit runner removes database target variables. Proxy and readiness-route tests separately protect the non-production gate, API firewall, and production-inert behavior.

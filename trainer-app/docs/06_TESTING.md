...
- Planned alternative-session coverage spans `src/lib/api/next-session.test.ts`, `src/lib/api/program.test.ts`, `src/app/api/workouts/generate-from-intent/route.test.ts`, `src/app/api/workouts/save/route.integration.test.ts`, and `src/components/IntentWorkoutCard.test.tsx`. It protects ordered eligible discovery, exact off-order slot generation, stale/completed/incomplete exclusion, unchanged schedule order, idempotent completion, next-slot recommendation, and the compact Home chooser interaction.

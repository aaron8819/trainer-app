...
The read-only V0 compiler proof is covered by `src/lib/api/plan-specification-preview-v0.test.ts`. It verifies stable canonical semantics and hashes, exact equality with the existing custom executable projection, ordering and role/set mapping, explicit defaults, invalid and unsupported input, current accepted-seed validation, truthful Plan Health inclusion, no persistence dependencies, and non-consumption by targeted runtime owners. Run the developer preview with `npm run preview:plan-specification-v0`; it is credential-free and performs no database reads or writes.

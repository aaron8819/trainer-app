...
- Performed-history progression coverage: `src/lib/engine/apply-loads.correctness.test.ts` and `src/lib/engine/history.test.ts` (includes `PARTIAL` and malformed legacy-status handling; also covers top-set anchor correctness, accumulation-anchored scheduled deload load-down with canonical fallback/history exclusion, and bodyweight early-exit behavior).

...
- V4 custom-plan acceptance/replay coverage: `src/lib/engine/hypertrophy-plan-authoring-v4.test.ts`, `src/lib/api/v4-prescription-normalization.test.ts`, `src/lib/api/hypertrophy-plan-drafts.test.ts`, `src/lib/api/mesocycle-seed-revision.test.ts`, `src/lib/api/slot-plan-seed-parser.test.ts`, `src/lib/api/template-session/slot-plan-seed.test.ts`, `src/lib/api/template-session.test.ts`, the plan-finalize and workout-generation route tests, and `src/components/plans/WeeklyHypertrophyPlanEditor.test.tsx`. These pin strict V4 parsing/hashing, confirmed-preview CAS acceptance, rollback, revision 1 authority, server-controlled copied-measurement provenance, fail-closed unrecognized limitations, exact-replay-only autoregulation isolation, rollout/write gates, UI handoff, and unchanged V1–V3 fixtures. The exhaustive production-materializer proof compares all 20 distinct reference-plan week/session combinations with the independently authored, test-owned matrix in `src/lib/api/template-session-v4-reference.expected.ts`; no expected prescription is resolved or normalized through production code. It covers prescribed and omitted placement identity, exercise count/identity/order, every materialized set and rep range, explicit RPE, complete measurement tuples, exact revision provenance, and absence of warm-ups, hip-flexor preparation, finisher composition, and selection fallback. Its mutation sentinel exercises the same comparison boundary for set count, reps, RPE, measurement, placement identity, and omission sensitivity. Separately, one real disposable PostgreSQL lifecycle in `scripts/test-v4-custom-plan-postgres.ts --confirm-disposable` proves persistence, transaction, activation, scheduling, and one scheduled materialization. It does not run 20 separate database lifecycles; the independent production-materializer matrix supplies the exhaustive prescription coverage.
...
Focused coverage lives in `hypertrophy-plan-authoring.test.ts`, `hypertrophy-plan-drafts.test.ts`, the accepted-revision/parser suites, runtime-swap suites, strength-policy suites, plan-management/activation suites, and the rollout test. It proves manual/V2 draft convergence, strict minimal intent with the bounded low-axial exercise-class semantic, shipped-catalog consistency across generated rows/picker/health/runtime, current accepted-revision precedence over compatibility seed metadata, semantic plan-health blockers, explicit no-intent legacy swap behavior, CAS save behavior, atomic draft consumption/rollback, lossless editable copy, projection alignment, version-dispatched hashing, exact legacy Strength equipment compatibility, role-aware runtime substitution, separate activation, and default-off rollout behavior.

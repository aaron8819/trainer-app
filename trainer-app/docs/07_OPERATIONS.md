...
- Before an expensive hermetic verification run, inspect exact-tree CI evidence as documented in `docs/06_TESTING.md`. Reuse a valid pass for the same tree and equivalent definition/classification/lockfile; a new operator or review session alone is not a rerun reason.

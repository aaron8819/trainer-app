...
- `plannerOnlyNoRepair.v2ConcentrationMaterializerProjection` is the flagged read-only materializer projection for a selected concentration warning. It applies a one-step lane set-cap trial only to an in-memory exercise-selection plan copy across representative accumulation weeks, reruns the V2 dry-run materializer for baseline versus trial, and reports identity/set/blocker deltas plus concentration warning/share/fatigue proxy deltas. The materialization shard carries the detailed projection when `--v2-debug-artifact` is used; the main/index output keeps compact status and deltas. A positive warning delta remains diagnostic until cross-week redistribution, acceptance, seed/runtime, and non-consumption gates are measured. This projection must not feed demand, weekly curve, slot allocation, canonical set policy, scoring, generation, selection-v2, repair, seed serialization, runtime replay, receipts, UI, persistence, DB, or live V2 writes.

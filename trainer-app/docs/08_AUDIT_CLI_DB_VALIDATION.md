...
- `plannerOnlyNoRepair.v2PlanQualityBenchmark` is the first read-only V2 candidate-quality benchmark. It evaluates the V2 candidate from first principles: support floors, direct work, lane preservation, session size, fatigue distribution, duplicate/concentration risk, materializer omissions, and Week 1 trainability. When pure V2 base-plan validation/compare evidence is available, the support-floor, direct-work, and lane-preservation gates use that candidate evidence before falling back to legacy no-repair projection readouts. Legacy no-repair projection failures remain evidence about the old path, not proof that the pure V2 candidate failed. Missing evidence stays `missing_evidence`; do not infer readiness from absent data. The benchmark reports deprecation readiness only as review guidance and keeps guardrails explicit: no seed/runtime change, no production materializer change, no acceptance-threshold change, and no persistence change. Repaired projection is evidence only, not target policy.

...
- `plannerOnlyNoRepair.v2PlanQualityBenchmark` is the first read-only V2 candidate-quality benchmark. It evaluates the current V2/no-repair candidate from first principles: support floors, direct work, lane preservation, session size, fatigue distribution, duplicate/concentration risk, materializer omissions, and Week 1 trainability. Missing evidence stays `missing_evidence`; do not infer readiness from absent data. The benchmark reports deprecation readiness only as review guidance and keeps guardrails explicit: no seed/runtime change, no production materializer change, no acceptance-threshold change, and no persistence change. Repaired projection is evidence only, not target policy.

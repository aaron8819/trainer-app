...
- `plannerOnlyNoRepair.strategyToDemandProjection.ownerScopedProjection` is the compact read-only follow-up for the current top owner row. It is currently scoped to `SlotDemandAllocationByWeek:Side Delts:protect_floor` and reports performed source evidence, affected slots/lanes/weeks, bounded delta status, slot-owned context, downstream slot/set context, materializer identity/set/blocker delta status, protected coverage impact, readiness, and proof still required before behavior. The companion `plannerOnlyNoRepair.v2StrategyRowMaterializerProjection` now measures that row through an in-memory slot-allocation copy: a `+1` floor-buffer trial on `week_1:upper_b:side_delt_isolation`, downstream class/capacity/selection rebuild, and baseline-versus-trial dry-run materializer comparison. The live result remains `blocked`, not diagnostic/no-impact or candidate-for-bounded-review: the audit line should show `projected-with-limitations`, readiness `blocked`, identity delta `0`, total set delta `0`, target lane set delta `-1`, blocker delta `0`, protected coverage `regressed`, loss cause `diagnostic-artifact`, concentration `improved`, and next safe slice `inspect-materializer-or-concentration-regressions`. The cause is the projection marker itself: `target_lane` switches the Side Delts lane from `support_direct_floor` to `class_ownership_allocation`, reducing the selected/materialized Side Delts lane from 4 to 3 while a collateral lane absorbs the offset. Full detail belongs in the `v2-materialization` debug shard and debug index; the main artifact keeps a compact summary. It must not feed demand, weekly curve, slot allocation, set distribution, materializer ranking, generation, repair, seed serialization, runtime replay, receipts, accepted mesocycle behavior, UI, API writes, DB, or persistence.

...
- `plannerOnlyNoRepair.v2ConcentrationMaterializerProjection` is the flagged read-only materializer projection for a selected concentration warning. It applies a one-step lane set-cap trial only to an in-memory exercise-selection plan copy across representative accumulation weeks, reruns the V2 dry-run materializer for baseline versus trial, and reports identity/set/blocker deltas plus concentration warning/share/fatigue proxy deltas. Its nested `donorOffsetRedistributionProjection` is the bounded read-only follow-up: it reads `SlotDemandAllocationByWeek` only to pick same-muscle slot-owned donor-offset lanes, applies paired source -1 / donor +1 lane-budget changes to an in-memory trial, and reports source/donor rows, protected coverage impact, materializer deltas, concentration warning delta, regression causes, alternate donor candidates, acceptance/trainability status, readiness, blockers, and `nextSafeSlice`. The materialization shard carries row detail when `--v2-debug-artifact` is used; the main/index output keeps compact status, deltas, cross-week readiness decision, blocker count, donor-offset status/readiness, alternate counts, regression-cause counts, and next safe slice. Current 2026-06-12 live evidence is blocked with no safe alternate donor: the Calves source lane reduces `lower_a:calves` by 1 set, the intended `lower_b:calves` donor does not gain a set, net weekly volume drops by 1 set, protected coverage regresses in Weeks 2-4, and no alternate donor candidates are measurable. `crossWeekReadiness` separates coverage, redistribution/donor-offset proof, no-repair acceptance/Week 1 trainability, materializer non-regression, duplicate/concentration non-regression, production materializer non-consumption, and seed/runtime/receipt/DB non-consumption. A positive single-lane warning delta remains diagnostic unless the cross-week rows and gates support a bounded policy-design slice; any protected-coverage, net-volume, identity/set/blocker, or concentration regression blocks behavior work. Donor-offset success still only advances to read-only acceptance/non-regression projection; it does not make the candidate behavior-ready. This projection must not feed demand, weekly curve, slot allocation, canonical set policy, scoring, generation, selection-v2, repair, seed serialization, runtime replay, receipts, UI, persistence, DB, or live V2 writes.

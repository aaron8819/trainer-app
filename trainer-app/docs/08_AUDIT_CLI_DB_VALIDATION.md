...
- The payload includes `plannerOnlyNoRepair.crossWeekProjectionGate`, which separates Week 1 basic shape, Weeks 2-4 accumulation projection, deload diagnostic projection, and broad replacement readiness. The main artifact keeps only compact gate statuses/counts; the V2 sidecar carries the full gate. Weeks 2-4 may report `projected_with_limitations` when the planner-owned read-only projection exists, but `safeToPromoteBehavior` remains `false` while that projection is not consumed by selection/seed/runtime, deload lacks seed/runtime projection, accepted seed/runtime consumption is undefined, or repair dependency could worsen.

...
Current V2 candidate-selection note, 2026-06-13: live `mesocycle-explain --planner-only-no-repair --compare-repaired` also reports `plannerOnlyNoRepair.v2PromotionCandidateEvaluator`. Read this compact evaluator before choosing more V2 work. It should rank only measured owner-specific evidence and say `none_ready` when the current queue is promoted baseline, measured no-impact, stale/noise, bounded watch without a bounded delta, diagnostic artifact, repair-only evidence, combined shadow evidence, or materializer regression. Its guardrails must remain `readOnly=true`, `affectsScoringOrGeneration=false`, `consumedByProduction=false`, `consumedByDemandOrMaterializer=false`, and repaired projection must be `evidence_only_not_target_policy`. Full candidate rows belong in the V2 promotion-readiness debug shard; the main artifact and CLI should stay compact.

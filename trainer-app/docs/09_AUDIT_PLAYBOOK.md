...
- `exerciseClassAlignment` is the compact read-only comparison of planner class intent, initial slot-selected exercise classes, and final repaired exercise classes. Read it to answer whether slot-local selection satisfied class intent, whether repair improved or worsened that alignment, whether identity churn occurred, and whether duplicates such as Incline DB Bench, Lat Pulldown, SLDL, Barbell Back Squat, or same-session calf isolation variants are class-aligned but duplicate-policy risky. Read the sibling `exerciseClassUnresolvedCauses` before proposing behavior: it classifies notable unresolved rows by likely owner so selection blind spots, inventory/classification gaps, duplicate/continuity conflicts, support-floor late repair, cap cleanup/final shaping, repair identity churn, true unresolved demand, and diagnostic-only rows do not get the same fix. Both fields are diagnostic-only and must not be used as active selection, repair, seed, receipt, UI, or runtime replay policy.

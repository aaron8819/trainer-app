...
Current V2 planner-policy interpretation, 2026-07-24: audit readouts must keep weekly biological demand, fractional slot attribution, and executable authored set budgets separate. A missing governing set-budget target is diagnostic-only (`setBudgetTargetEvidence:diagnostic_only`) and cannot invent an under-budget warning or failure; an observed missing required direct class remains canonical deficit evidence. A normal in-budget 4-set anchor is not a concentration warning by itself. Rear Delt materialization proof is scoped to the Rear Delt direct-class and support-floor rows, so unrelated global regression totals remain visible but do not negate that local proof. Repaired projection remains evidence only, never target policy.

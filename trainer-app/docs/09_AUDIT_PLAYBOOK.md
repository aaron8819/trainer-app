...
Current V2 readout note, 2026-06-13: when reading `mesocycle-explain` with V2 debug artifacts, treat the promoted Weeks 2-4 Calves 3/5 allocation as baseline evidence. `repairPromotionScoreboard.interpretation.gapInventory` should show `concentration_quality` as `measured_promoted_baseline_idempotent`, the benchmark fatigue/concentration warning should remain a bounded-owner watch, and row proof belongs in the `v2-materialization` shard/index rather than the main artifact. For support-floor/readout evidence, read the compact readout bucket beside the raw source label: `direct_floor_below` explains what was observed, while `measured_no_impact` or `stale_noise` prevents that row from becoming a false owner-policy candidate. For strategy-to-demand inventory, the latest owner-scoped Biceps projection reports `SlotDemandAllocationByWeek:Biceps:protect_floor` as diagnostic no-impact: the `+1` `week_1:upper_b:biceps` trial materializes `Barbell Curl:2 -> Barbell Curl:2` and preserves selected identity, total sets, target-lane sets, materializer blockers, protected coverage, concentration, and support-floor set-budget basis. The earlier Side Delts protect-floor row is also measured no-impact. Do not treat either row as materializer ranking, taxonomy, or production slot-allocation evidence.

Current V2 candidate-selection note, 2026-06-13: after reading the V2 plan-quality benchmark, read `plannerOnlyNoRepair.v2PromotionCandidateEvaluator` before picking a next planner slice. The evaluator is the compact work queue: it merges strategy-to-demand inventory, clean preselection feasibility, benchmark watches, repair-scoreboard gap inventory, selected proof rows, and materializer projections into ranked candidates with explicit stop reasons. `none_ready` is the correct recommendation when the queue is already-promoted baseline, measured no-impact, stale/noise, bounded watch without a measured bounded delta, diagnostic artifact, repair-only evidence, combined shadow evidence, or materializer regression. The Biceps protect-floor projection now measures baseline `Barbell Curl:2` versus trial `Barbell Curl:2` with identity/set/lane/blocker deltas `0`, protected coverage preserved, concentration preserved, acceptance proof missing, and next `pivot_to_higher_roi_track`; treat that row as measured no-impact, not fresh missing-evidence work. The lower-b Hamstrings clean-preselection row is the same class of stopped evidence (`Seated Leg Curl:3 -> Seated Leg Curl:3`). `blocked_by_missing_evidence` is the correct recommendation when remaining fresh owner-specific rows still lack bounded projection/materializer delta, acceptance/watch clearance, or seed/runtime/receipt/DB non-consumption proof. Open the `v2-promotion-readiness` debug shard only for row detail and source attribution. Do not use the evaluator as planner policy, acceptance scoring, materializer input, seed/runtime metadata, or repair behavior.

Current V2 lane-intent benchmark note, 2026-06-13: the plan-quality benchmark now includes `lane_intent_explicitness`, sourced from `V2LaneSelectionIntentAudit` and the pure `buildV2LaneSelectionIntentBenchmark()` expectation model. Read it as candidate-quality evidence for high-risk jobs: calf direct, hamstring curl, side/rear delt direct, chest-biased press support, vertical-pull anchor, and optional low-axial hip extension. A warning here is a bounded lane-intent contract watch, not an instruction to change materializer ranking. Required lane jobs currently pass; low-axial hip extension is a diagnostic watch. The expected contract is support coverage with required role `support_coverage`, future movement pattern `low_axial_hip_extension`, future class `low_axial_hip_extension_anchor`, low axial fatigue, moderate-or-high loadability, direct-or-high-support delivery, meaningful Glutes stimulus, Hamstrings support/collateral only, clean variation before same-family reuse, allowed hip-thrust/bridge/pull-through/reverse-hyper family, and explicit disallowance of true hinge overload, knee-flexion curl or hamstring-curl substitution, back-extension/lower-back-heavy closure, and generic glute accessory work. Taxonomy now recognizes Reverse Hyperextension as `low_axial_hip_extension_anchor`, but `laneSelectionIntent v0` still cannot express the future low-axial movement/class fields. Failure means unresolved planner-owned lane-intent contract; keep the diagnostic until a later measured materializer projection proves value. The gate remains read-only and must not feed demand, materializer behavior, seed/runtime/receipts, acceptance thresholds, repair behavior, DB writes, or persistence.

Current low-axial projection note, 2026-06-13: the later measured projection now exists as `plannerOnlyNoRepair.v2LaneIntentMaterializerProjection`. It applies the exact low-axial `support_coverage` contract to `lower_b:hinge_anchor` through an audit-only selection-plan class override, not through production `laneSelectionIntent v0` or the materializer allowlist. The live comparison measured `Stiff-Legged Deadlift:3 -> Cable Pull-Through:3` on the hinge lane, preserved `Seated Leg Curl:3`, kept total sets and materializer blockers unchanged, improved low-axial closure and protected Glutes low-axial sets (`0 -> 3`), reduced high-fatigue lower-b sets by 3, and preserved the exclusions for true hinges, hamstring curls, back-extension closure, and generic glute accessories. The companion `plannerOnlyNoRepair.v2PlanQualityBenchmark.laneIntentAcceptanceProjection` now classifies the bounded trial as `accepted_with_watch_items`: no Week 1 must-fix blocker, no session-size/set/blocker regression, protected Glutes/Hamstrings/lower-body coverage preserved or improved, direct/support floors preserved, and seed/runtime/receipt/DB/repair non-consumption still proven. Treat this as positive no-drift architecture evidence that graduates to a later behavior-design slice, not a production behavior change. The watch items are bounded to the planner-owned low-axial lane-intent contract and materializer-consumption design; do not feed the trial into seed/runtime/receipts/DB/repair/acceptance thresholds.

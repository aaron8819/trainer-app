...
- Safe Promotion Trial Protocol for shadow-owned demand promotion:
  ...
  6. Revert when `materialRepairCount`, `majorRepairCount`, or suspicious repair count increases; when `lower_b Chest via Cable Crossover` or a similar bad repair becomes planned policy; when set concentration worsens materially; or when new cross-region demand appears.

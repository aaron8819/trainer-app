...
- Read `plannerOnlyNoRepair.repairPromotionScoreboard` as a classification readout over existing repaired-planning repair evidence, not as behavior approval. Raw counts and suspicious rows stay intact; promotion candidates are limited to positive slot-owned likely-avoidable rows, while cap trim/removal, forbidden cleanup, collateral, and materiality-none rows stay in safety/diagnostic buckets.

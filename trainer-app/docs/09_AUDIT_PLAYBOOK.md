...
- Focused shards currently split V2 diagnostics into `v2-strategy`, `v2-promotion-readiness`, `v2-promotion-diffs`, `v2-repair-evidence`, `v2-materialization`, `v2-cross-week-projection`, and `v2-selection-alignment`. Summary fields stay in the main artifact and index; large arrays are omitted or summarized in compact shards, and repeated repair evidence is cataloged once and referenced by ID where possible.
- Artifact budgets are explicit: main artifact 1 MiB, V2 index 128 KiB, default shard 512 KiB, full-detail shard 1 MiB, and per-artifact limit 1 MiB. Default shards are kept below the compact budget individually.
...
- `strategyHypothesisPromotionDiff` is the read-only gate after promotion readiness. It currently evaluates `protect_lagging_muscles_earlier` and `cap_late_block_volume` only when ready for read-only diff, uses target-tier under-hit evidence for lagging-muscle protection, uses skipped-set plus hard-week effort evidence for late-block caps, and surfaces the tension between protection and total-volume caps. Its nested `projectionDiff` estimates the combined pair against the V2 planner target without changing behavior: it prefers redistribution from supported over-concentration/fatigue-driver muscles before net-new late-block volume, lists candidate protected/donor counts, and computes non-regression gates as `pass` / `fail` / `unknown` from projected deltas. If there is no true shadow projection, do not treat hypothesis presence or positive evidence quality as a passing gate; unknown gates should remain unknown. The operator summary prints evaluated hypotheses, target-tier examples, hard-week skipped-set signal, interaction risk, legacy gate reporting, projection status/mode, candidate protected/donor counts, computed gate counts, readiness, limitations, next safe action, and `consumedByDemandOrMaterializer=false`; detailed arrays stay in `v2-promotion-diffs`, not the main artifact.
...
- `plannerOnlyNoRepair.v2SelectionCapacityPlanDiagnostic` is a flagged read-only capacity/headroom diagnostic. The selection-alignment shard carries its compact status, summary, and counts by default. Read its summary counts, blockers/warnings/missing inputs, and per-week lane classifications to separate target-met upper-pull session-cap pressure, Week 4 calf cap-aware expansion limits, optional suppressed lanes, and true below-min target-unmet blockers. It is read-only evidence only and must not promote behavior or feed selection, repair, seed, runtime replay, UI, receipts, or persistence.

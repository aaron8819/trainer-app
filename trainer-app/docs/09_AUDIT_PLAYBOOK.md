...
- `strategyHypothesisPromotionDiff` is the read-only gate after promotion readiness. It currently evaluates `protect_lagging_muscles_earlier` and `cap_late_block_volume` only when ready for read-only diff, uses target-tier under-hit evidence for lagging-muscle protection, uses skipped-set plus hard-week effort evidence for late-block caps, surfaces the tension between protection and total-volume caps, and reports non-regression gates without enforcing behavior. The operator summary prints evaluated hypotheses, target-tier examples, hard-week skipped-set signal, interaction risk, gate status, next safe action, and `consumedByDemandOrMaterializer=false`; detailed arrays stay in `v2-promotion-diffs`, not the main artifact.

...
- Focused shards currently split V2 diagnostics into `v2-strategy`, `v2-promotion-readiness`, `v2-promotion-diffs`, `v2-repair-evidence`, `v2-materialization`, `v2-cross-week-projection`, and `v2-selection-alignment`. Summary fields stay in the main artifact and index; large arrays are omitted or summarized in compact shards, and repeated repair evidence is cataloged once and referenced by ID where possible. The V2 base-plan compare is summarized in the main artifact/CLI, but its detailed slot shape, muscle/class coverage, repair dependency, exercise identity, and deload-readiness rows belong in `v2-materialization`.

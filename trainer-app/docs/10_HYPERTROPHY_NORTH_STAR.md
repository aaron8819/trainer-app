...
1. Evaluate `plannerOnlyNoRepair.v2PlanQualityBenchmark` first. Its gates are support floors, direct work, lane preservation, session size, fatigue distribution, duplicate/concentration risk, materializer omissions, and Week 1 trainability. For support floors, direct work, lane preservation, session size, and duplicate/concentration risk, read-only pure V2 base-plan evidence is the candidate-quality source when present; legacy no-repair projection remains fallback evidence about the old path. Session-size warnings should reflect pure slot-shape risk, while duplicate/concentration warnings should name exact reuse as a watch item unless regression evidence exists. Class-family-only reuse can be explicitly bounded when exact duplicate reuse is zero and base-plan regressions are zero. For concentration materializer trials, read `concentrationReadinessDecision`, `donorOffsetRedistributionProjection.summary`, and `slotWeekAllocationProjection.summary` before proposing policy: a single positive warning delta is not enough; representative Weeks 2-4 materializer rows, donor-offset redistribution, donor absorption, protected coverage, acceptance/non-regression, and seed/runtime/receipt/DB non-consumption decide whether the next move is bounded design, blocker investigation, diagnostic-only measurement, or pivot. The 2026-06-13 live slot/week allocation policy trial proves exact donor absorption before materialization: Calves Weeks 2-4 move `lower_a:calves` 4 to 3 and `lower_b:calves` 4 to 5 with zero net weekly delta, preserved protected coverage, no materializer regression, and no over/under-absorption rows. The prior Week 4 blocker was a fractional rounding problem (`3.5/5.5` became `4/6` after set distribution); the read-only `SlotDemandAllocationByWeek` trial now carries exact downstream baseline set targets before the downstream rebuild. The first-principles design rule remains explicit: `SlotDemandAllocationByWeek` may relieve concentration pressure only when a same-muscle slot-owned donor measurably absorbs the required protected set without net weekly volume loss, protected coverage regression, materializer regression, or seed/runtime consumption. The current readout is `slotWeekAllocationReadiness=candidate_for_acceptance_projection`, `blockedRows=0`, `slotWeekAllocationNextSafeSlice=run_acceptance_non_regression_projection`, and `slotWeekAllocationAcceptanceProjection.decision=accepted_with_watch_items` with benchmark `pass=4 warn=4 fail=0 missing=0`. Current watch classification is `accepted=6 boundedOwner=2 ownerFix=0 staleNoise=1 blockers=0`, so the readiness decision is `ready_for_bounded_promotion_slice`: the next move can be a future bounded behavior-promotion implementation design slice, but this review still does not promote production behavior. The stale standalone `base-plan-validation.test.ts` fixture has been reconciled to the current clean V2 base shape (`64` sets, `20` exercises, no optional triceps materialization), so it is not pre-promotion cleanup. Donor-offset success still only advances to bounded behavior design, not behavior promotion.
   - Bounded implementation target: promote only this Calves redistribution in pure `SlotDemandAllocationByWeek`, after base per-week slot exposure allocation and before class/set/capacity/selection planning. The policy must be explicit, not generalized concentration relief: accumulation Weeks 2-4, `lower_a:calves` source drops from 4 to 3, `lower_b:calves` donor rises from 4 to 5, same muscle/lane only, slot-owned donor required, and net weekly Calves sets remain unchanged. The production helper should be small enough to delete as rollback and must not read audit diagnostics, repaired projection, materializer lane IDs, acceptedPlannerIntent, seed JSON, receipts, runtime state, UI, or DB. The first implementation slice should update `src/lib/engine/planning/v2/slot-demand-allocation.ts` and `slot-demand-allocation.test.ts`; any downstream test updates should only prove normal pure V2 consumption of the adjusted allocation and boundary non-consumption.

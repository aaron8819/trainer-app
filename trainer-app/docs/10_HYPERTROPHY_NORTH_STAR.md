...
- Current Side Delts strategy-row checkpoint, 2026-06-13: the read-only materializer projection for `SlotDemandAllocationByWeek:Side Delts:protect_floor` is measured faithfully and is now diagnostic no-impact. The bounded projection-only `+1` floor-buffer trial on `week_1:upper_b:side_delt_isolation` rebuilds downstream class/capacity/selection policy and reruns the dry-run materializer while preserving the original support-floor budgeting semantics. Live evidence shows allocation basis preserved, set-budget basis `support_direct_floor -> support_direct_floor`, selected budget `4 -> 4`, identity delta `0`, total set delta `0`, target-lane set delta `0`, blocker delta `0`, protected coverage `preserved`, concentration `preserved`, and `basisChanged=false`. This is not a behavior candidate; the next safe slice is to pivot to a higher-ROI owner row or broader candidate-evaluator cleanup unless new performed evidence identifies a measurable owner-specific gap. Production allocation, seed/runtime replay, receipts, DB, and UI remain unchanged.
- Current V2 candidate evaluator checkpoint, 2026-06-13: `plannerOnlyNoRepair.v2PromotionCandidateEvaluator` is the reusable read-only ROI filter for future planner work. It ranks existing diagnostics by measured owner-specific positive impact, materializer non-regression, protected coverage, acceptance/watch status, seed/runtime/receipt/DB non-consumption, source attribution quality, prior probe result, and implementation scope. It should recommend the next safe V2 slice only when those gates produce a real measured candidate; otherwise it should say `none_ready` and list stopped candidates. The current known queue is stopped or watch-only: Calves is already promoted baseline, support-floor is measured no-impact/stale cleanup, duplicate/class-family is a bounded watch without a new bounded delta, lane-preservation is stale/noise, Side Delts protect-floor is measured diagnostic no-impact, and lower-b Hamstrings clean preselection is measured diagnostic no-impact (`Seated Leg Curl:3 -> Seated Leg Curl:3`, zero identity/set/lane/blocker deltas, protected coverage preserved). Repaired projection remains evidence only, not target policy.

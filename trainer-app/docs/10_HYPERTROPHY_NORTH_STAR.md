...
- Current V2 lane-intent benchmark checkpoint, 2026-06-13: the first ontology/laneSelectionIntent v0 benchmark slice is read-only and candidate-quality-only. `buildV2LaneSelectionIntentBenchmark()` checks high-risk lane jobs for explicit required/preferred/disallowed movement and class semantics, directness, stability/fatigue/loadability preferences, duplicate/fallback policy, identity preservation, and failure meaning. Calf direct, hamstring curl, side-delt direct, rear-delt direct, chest-biased press support, and vertical-pull anchor currently pass the explicitness model; low-axial hip extension remains a diagnostic warning because the planner has not yet separated low-axial posterior-chain support from hinge overload. The taxonomy now recognizes Reverse Hyperextension as `low_axial_hip_extension_anchor`. The future low-axial contract is a support-coverage lane job with role `support_coverage`, movement pattern `low_axial_hip_extension`, class `low_axial_hip_extension_anchor`, preferred hip-thrust/bridge/pull-through/reverse-hyper patterns, allowed low-axial hip-extension family, disallowed true hinge overload, hamstring curl or knee-flexion curl substitution, back-extension/lower-back-heavy closure, and generic glute accessory work, meaningful Glutes stimulus, Hamstrings support/collateral only, low axial fatigue, moderate-or-high loadability, direct-or-high-support delivery, low-axial substitution family, and clean variation before same-family reuse. Failure means unresolved planner-owned lane-intent contract rather than materializer ranking failure. This is a lane-intent contract work queue; a later behavior slice would first extend `laneSelectionIntent`/`ExerciseSelectionPlan`, then run a measured materializer projection before considering any production ranking consumption.
- Current low-axial materializer projection checkpoint, 2026-06-13: the measured audit-only projection has now run without promoting behavior. `plannerOnlyNoRepair.v2LaneIntentMaterializerProjection` applies the exact low-axial support-coverage contract to `lower_b:hinge_anchor` as a trial selection-plan class override (`low_axial_hip_extension_anchor`) while leaving `laneSelectionIntent v0`, production materializer ranking, and the consumed allowlist unchanged. The live comparison changed the lower-b hinge lane from `Stiff-Legged Deadlift:3` to `Cable Pull-Through:3`, preserved `Seated Leg Curl:3`, kept total sets and materializer blockers unchanged, improved low-axial closure and Glutes low-axial coverage (`0 -> 3` sets), reduced high-fatigue lower-b sets by 3, and selected no true hinge overload, hamstring curl substitution, back-extension closure, or generic glute accessory. `plannerOnlyNoRepair.v2PlanQualityBenchmark.laneIntentAcceptanceProjection` now accepts the trial with watch items: Week 1 has no must-fix blocker, session size and floor preservation pass, protected coverage and materializer non-regression pass, and seed/runtime/receipt/DB/repair non-consumption remains proven. This graduates low-axial to a later bounded behavior-design slice, where the owner remains planner-owned lane intent plus explicit materializer consumption; it is not production behavior in this diagnostic slice.

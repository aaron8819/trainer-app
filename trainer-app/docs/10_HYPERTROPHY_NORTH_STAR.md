...
- Current Side Delts strategy-row checkpoint, 2026-06-13: the read-only materializer projection for `SlotDemandAllocationByWeek:Side Delts:protect_floor` is measured but blocked. The bounded projection-only `+1` floor-buffer trial on `week_1:upper_b:side_delt_isolation` rebuilds downstream class/capacity/selection policy and reruns the dry-run materializer, but live evidence shows identity delta `0`, total set delta `0`, target-lane set delta `-1`, blocker delta `0`, protected coverage `regressed`, and concentration `improved`. Cause classification is `diagnostic_artifact`: the projection-only `target_lane` marker changes the Side Delts set-budget basis from `support_direct_floor` to `class_ownership_allocation`, reducing the selected/materialized target lane from 4 to 3 while another upper_b lane absorbs the set. This is not a behavior candidate; the next safe slice is diagnostic-trial/protected-coverage cause cleanup plus acceptance/non-consumption proof, not production allocation or seed/runtime change.
...
The next true planner-intelligence seam is the read-only strategy-to-demand candidate inventory, not more repair/readout cleanup. `V2PlannerMesocyclePolicy.strategyToDemandProjection.candidateInventory` should identify whether performed reality, benchmark watches, no-repair projection evidence, or repair-only evidence imply future owner-specific changes to `MesocycleDemand`, `WeeklyDemandCurve`, `SlotDemandAllocationByWeek`, or `SetDistributionIntent`. Its rows preserve source attribution, name affected muscles/slots/lanes/weeks when known, classify readiness as blocked, diagnostic-only, or candidate-for-read-only-projection, and list required proof before behavior. The companion `ownerScopedProjection` and `v2StrategyRowMaterializerProjection` have now measured the current top row, `SlotDemandAllocationByWeek:Side Delts:protect_floor`, as still blocked: source evidence and upper-slot context exist, downstream class/capacity/selection/materializer deltas are measured, but the bounded `+1` trial regresses protected target-lane coverage while leaving identity, total sets, and blockers flat. The measured cause is diagnostic-artifact ownership in the trial construction/readout path, not materializer ranking or taxonomy. The first useful recommendation should come from performed-reality floor/protection or cap/redistribution evidence that has owner context and can pass read-only materializer, protected-coverage, acceptance, and non-consumption gates; repaired projection remains evidence only and never target policy.

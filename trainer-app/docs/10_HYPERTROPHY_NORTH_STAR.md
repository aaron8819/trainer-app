...
Current approved balanced four-day V2 direct-volume policy, 2026-07-24: the planner treats volume as preferred work plus capacity floors, not six independent hard quotas. Preferred direct sets for Hamstrings / Side Delts / Rear Delts / Triceps / Biceps / Calves are `8/8/4/4/2/8`; floors are `6/4/2/2/0/6`; minimum direct exposures are `2/1/1/1/0/2`. Hinge plus curl, one Side Delt isolation exposure, one Rear Delt isolation exposure, one Triceps isolation exposure, and two Calf exposures are structural. The second Side Delt exposure, Biceps, and surplus direct work are preferred and yield under session capacity. The preferred representative Week 2 remains 67 sets / 21 exercises at `21/11/21/14`; moderate and minimal fixtures are 59 and 50 sets. A 45-minute session is constrained capacity and should not assume the full 67-set template. Upper B Triceps is a conditional recoverable top-up toward six total sets after bounded pressing collateral, never a hard direct floor and never a substitute for direct isolation. `acceptedPlannerIntent` may store this policy as explanatory metadata, but executable seed rows remain `exerciseId` / `role` / `setCount`; runtime replay, receipts, repair, API/UI, persistence behavior, and DB behavior do not consume the added fields.

Current Stage 1 product-authoring checkpoint, 2026-07-25: next-mesocycle setup exposes Efficient, Balanced, and Full as product choices mapped canonically to the existing `minimal`, `moderate`, and `preferred` capacity profiles. Balanced is the no-preference default. The choice is available only for an explicitly confirmed four-day Upper/Lower candidate, reaches the real V2 refresh/materialization policy, invalidates mismatched refreshed drafts, and is accepted only from the exact refreshed candidate. Product choice and recommendation context remain explanatory accepted intent; executable seed and runtime replay are unchanged. Automatic readiness/history adaptation, session overrides, duration estimation, user defaults, and generalized three-day planning remain deferred Stage 2/3 work, not current behavior.
...
- Current V2 readout cleanup, 2026-06-13: Weeks 2-4 `lower_a:calves` 4 -> 3 and `lower_b:calves` 4 -> 5 are promoted baseline planner policy. Remaining materializer/readout diagnostics should report that exact shape as idempotent bounded-owner evidence, not a new slot-allocation target or no-impact pressure. Support-floor/readout diagnostics should classify measured zero-delta audit-cleanup rows as measured no-impact while preserving their raw source attribution.
- Current strategy-row checkpoints, 2026-06-13: the read-only materializer projection for `SlotDemandAllocationByWeek:Biceps:protect_floor` is now measured and diagnostic no-impact. The bounded projection-only `+1` floor-buffer trial on `week_1:upper_b:biceps` rebuilds downstream class/capacity/selection policy and reruns the dry-run materializer. Live evidence shows selected/materialized Biceps `Barbell Curl:2 -> Barbell Curl:2`, set-budget basis `support_direct_floor -> support_direct_floor`, identity delta `0`, total set delta `0`, target-lane set delta `0`, blocker delta `0`, protected coverage `preserved`, and concentration `preserved`. The earlier `SlotDemandAllocationByWeek:Side Delts:protect_floor` row is also measured diagnostic no-impact. Neither row is a behavior candidate; production allocation, seed/runtime replay, receipts, DB, and UI remain unchanged.
- Current V2 candidate evaluator checkpoint, 2026-06-13: `plannerOnlyNoRepair.v2PromotionCandidateEvaluator` is the reusable read-only ROI filter for future planner work. It ranks existing diagnostics by measured owner-specific positive impact, materializer non-regression, protected coverage, acceptance/watch status, seed/runtime/receipt/DB non-consumption, source attribution quality, prior probe result, and implementation scope. It should recommend the next safe V2 slice only when those gates produce a real measured candidate; otherwise it should say `none_ready` or `blocked_by_missing_evidence` with explicit stopped and blocked candidates. The current known stopped rows include Calves already-promoted baseline, support-floor measured no-impact/stale cleanup, lane-preservation stale/noise, Side Delts protect-floor measured diagnostic no-impact, Biceps protect-floor measured diagnostic no-impact, and lower-b Hamstrings clean preselection measured diagnostic no-impact (`Seated Leg Curl:3 -> Seated Leg Curl:3`). Duplicate/class-family remains bounded watch evidence without a new bounded delta. Repaired projection remains evidence only, not target policy.
- Current live benchmark checkpoint, 2026-06-15: `plannerOnlyNoRepair.v2PlanQualityBenchmark` is now `pass=5 warn=4 fail=0 missing=0 mustFixW1=0`. The low-axial/base-plan fix moved `materializer_omissions` to `pass` from `pure_v2_base_plan`; low-axial support coverage and materializer omissions are no longer active default-author blockers. Promotion is still blocked by missing bounded owner-specific evidence because the live architecture remains `mostly_repair_shaped` with raw repair pressure and suspicious downstream repair rows quarantined from promotion.
- Current default-author readiness checkpoint, 2026-06-15: `plannerOnlyNoRepair.v2DefaultAuthorReadinessMap` is the compact concept-level navigation layer over the V2 factory line. It classifies `MesocycleDemand`, `WeeklyDemandCurve`, `SlotDemandAllocationByWeek`, `SetDistributionIntent`, `ExerciseClassDistributionBySlot`, `ExerciseSelectionPlan / selection capacity`, `V2 materializer`, and `Acceptance / promotion readiness` by owner seam, evidence source, readiness, blocker category, and next safe action. It is read-only audit infrastructure: no planner/materializer behavior, seed/runtime replay, receipts, persistence, acceptance thresholds, repair behavior, or DB state consume it. Repair rows remain safety-net symptoms unless the evaluator has already mapped them to a concept-level owner/proof blocker; the map is not a row-heavy work queue.
- Current V2 lane-intent benchmark checkpoint, 2026-06-13: the ontology/laneSelectionIntent v0 benchmark slice is read-only and candidate-quality-only. `buildV2LaneSelectionIntentBenchmark()` checks high-risk lane jobs for explicit required/preferred/disallowed movement and class semantics, directness, stability/fatigue/loadability preferences, duplicate/fallback policy, identity preservation, and failure meaning. Calf direct, hamstring curl, side-delt direct, rear-delt direct, vertical-pull anchor, and low-axial hip-extension support now pass the explicitness model. Vertical Press remains class-owned by `vertical_press`; Chest support ownership does not create a chest-biased benchmark obligation. The low-axial contract is a production-consumed support-coverage lane job for `lower_b:hinge_anchor` with movement pattern `low_axial_hip_extension`, class `low_axial_hip_extension_anchor`, meaningful Glutes stimulus, Hamstrings support/collateral only, low axial fatigue, moderate-or-high loadability, direct-or-high-support delivery, clean variation preference, and explicit exclusion of true hinge overload, hamstring curl or knee-flexion curl substitution, back-extension/lower-back-heavy closure, and generic glute accessory work. Failure means the materializer should expose a blocker rather than silently substituting a different posterior-chain job.
- Current V2 candidate-quality lab fixture checkpoint, 2026-06-13: `buildV2CandidateQualityLabFixtures()` is the first reusable read-only lab fixture layer over the lane-intent benchmark. It keeps the low-axial golden case stable and adds five named diagnostic scenarios for vertical-pull anchor, hamstring curl direct, side/rear delt direct, and calf direct, for six scenarios total. Chest-biased press support is intentionally absent because Vertical Press retains its own class obligation. Each scenario records equipment/inventory constraints, expected and actual pass/warn/fail/watch outcome, owner seam, evidence source, observed gap kind, and next safe action. The first materializer-delta layer starts with low-axial golden plus vertical-pull anchor, reporting pure baseline/trial materialized-plan identities, set and blocker deltas, protected coverage, non-consumption, and next safe action separately from semantic pass/fail status. It separates ontology, lane-contract, materializer-ranking, acceptance/watch, and seed/runtime-boundary issues while staying non-consuming: no production planner/materializer ranking change, no seed/runtime/receipt/DB change, no acceptance-threshold change, and no repair behavior change.
- Current low-axial materializer behavior checkpoint, 2026-06-13: the bounded production slice is implemented in pure V2 lane intent, exercise selection, and materializer consumption only. `plannerOnlyNoRepair.v2LaneIntentMaterializerProjection` now compares a legacy/no-intent baseline against the current consumed low-axial intent, preserving the same non-regression checks without making audit output executable policy. The owner remains `V2LaneSelectionIntent -> ExerciseSelectionPlan -> V2 materializer consumption`; seed shape, runtime replay, receipts, DB writes, repair behavior, UI, and acceptance thresholds remain unchanged.
...
- current V2 benchmark status is `pass=5 warn=4 fail=0 missing=0 mustFixW1=0`; `materializer_omissions` now passes from `pure_v2_base_plan`, so low-axial/base-plan omissions are resolved benchmark evidence
...
- the selected support-floor proof is `week_1:upper_a:side_delt_isolation:side_delts`, direct floor `0/4`, likely owner `audit_readout_cleanup`, raw evidence `direct_floor_below`, readout classification `measured_no_impact`, selected-proof classification `diagnostic_only_no_impact`, with `consumedByProduction=false` and `safeForBehaviorPromotion=false`; the next safe slice should move past stale/no-impact support-floor pressure before changing SetDistributionIntent or support policy
...
The next true planner-intelligence seam is the read-only strategy-to-demand candidate inventory, filtered by the promotion-candidate evaluator, not more repair/readout cleanup. `V2PlannerMesocyclePolicy.strategyToDemandProjection.candidateInventory` should identify whether performed reality, benchmark watches, no-repair projection evidence, or repair-only evidence imply future owner-specific changes to `MesocycleDemand`, `WeeklyDemandCurve`, `SlotDemandAllocationByWeek`, or `SetDistributionIntent`. Its rows preserve source attribution, name affected muscles/slots/lanes/weeks when known, classify readiness as blocked, diagnostic-only, or candidate-for-read-only-projection, and list required proof before behavior. `plannerOnlyNoRepair.v2PromotionCandidateEvaluator` then decides whether any row has enough measured owner-specific value to become the next safe slice. The companion `ownerScopedProjection` and `v2StrategyRowMaterializerProjection` have now measured the current top row, `SlotDemandAllocationByWeek:Side Delts:protect_floor`, as faithful no-impact evidence: source evidence and upper-slot context exist, downstream class/capacity/selection/materializer deltas are measured, the bounded `+1` trial preserves support-floor budgeting, target-lane coverage does not regress, and no collateral lane absorbs the set. The first useful recommendation should come from a different performed-reality floor/protection or cap/redistribution row that has owner context and can pass read-only materializer, protected-coverage, acceptance, and non-consumption gates; repaired projection remains evidence only and never target policy.
...
Historical verification record for that completed slice, preserved verbatim: focused materializer, taxonomy, base-plan, and architecture tests; `npx tsc --noEmit`; `npm run verify:contracts`; and `npm run verify`.
...
1. Evaluate `plannerOnlyNoRepair.v2PlanQualityBenchmark` first. Its gates are support floors, direct work, lane preservation, session size, fatigue distribution, duplicate/concentration risk, materializer omissions, and Week 1 trainability. For support floors, direct work, lane preservation, session size, duplicate/concentration risk, and materializer omissions, read-only pure V2 base-plan evidence is the candidate-quality source when present; legacy no-repair projection remains fallback evidence about the old path. Session-size warnings should reflect pure slot-shape risk, while duplicate/concentration warnings should name exact reuse as a watch item unless regression evidence exists. Class-family-only reuse can be explicitly bounded when exact duplicate reuse is zero and base-plan regressions are zero. For concentration materializer trials, read `concentrationReadinessDecision`, `donorOffsetRedistributionProjection.summary`, and `slotWeekAllocationProjection.summary` before proposing policy: a single positive warning delta is not enough; representative Weeks 2-4 materializer rows, donor-offset redistribution, donor absorption, protected coverage, acceptance/non-regression, and seed/runtime/receipt/DB non-consumption decide whether the next move is bounded design, blocker investigation, diagnostic-only measurement, or pivot. The 2026-06-13 live slot/week allocation policy trial proved exact donor absorption before materialization: Calves Weeks 2-4 move `lower_a:calves` 4 to 3 and `lower_b:calves` 4 to 5 with zero net weekly delta, preserved protected coverage, no materializer regression, and no over/under-absorption rows. That bounded redistribution is now implemented in pure `SlotDemandAllocationByWeek`, after base per-week slot exposure allocation and before class/set/capacity/selection planning. The first-principles design rule remains explicit: `SlotDemandAllocationByWeek` may relieve concentration pressure only when a same-muscle slot-owned donor measurably absorbs the required protected set without net weekly volume loss, protected coverage regression, materializer regression, or seed/runtime consumption. The current post-low-axial/base-plan readout remains `slotWeekAllocationReadiness=candidate_for_acceptance_projection`, `blockedRows=0`, `slotWeekAllocationNextSafeSlice=run_acceptance_non_regression_projection`, and `slotWeekAllocationAcceptanceProjection.decision=accepted_with_watch_items` with benchmark `pass=5 warn=4 fail=0 missing=0 mustFixW1=0`; `materializer_omissions` is the additional pass from `pure_v2_base_plan`. Current watch classification is `accepted=6 boundedOwner=2 ownerFix=0 staleNoise=1 blockers=0`. The stale standalone `base-plan-validation.test.ts` fixture has been reconciled to the current V2 base shape (`67` sets, `20` exercises, no optional triceps materialization, one flat-allocation quality warning), so it is not cleanup. Remaining concentration rows after this slice are readout/materializer cleanup unless a new measured owner-specific gate proves otherwise.
   ...
   - Promotion gates: all bounded readiness evidence must still pass immediately before and after implementation: donor `+1`, source `-1`, `netWeeklySetDelta=0`, protected coverage preserved, materializer identity/set/blocker non-regression, exact duplicate count clean, `mustFixW1=0`, benchmark `pass=5 warn=4 fail=0 missing=0`, watch classification `accepted=6 boundedOwner=2 ownerFix=0 staleNoise=1 blockers=0`, and seed/runtime/receipt/DB/materializer production non-consumption. Failure of any gate is a final blocker for the code slice and requires returning to read-only diagnostics.
   - Verification and rollout: from `trainer-app/`, with dependencies already installed, run focused pure V2 slot allocation tests, downstream class/set/materializer/benchmark tests, the V2 architecture-boundary non-consumption test, `npm run verify`, and the read-only live mesocycle-explain audit with `--planner-only-no-repair --compare-repaired --no-artifact`. If dependencies are missing, run `npm ci` deliberately; do not use a network-capable fallback. `npm run verify` includes the dependency preflight and repository-local TypeScript check through `npm run verify:fast`. Roll out as a single bounded code review with no DB writes, migrations, reseeds, repair/backfill, acceptance-threshold edits, seed-shape edits, runtime replay edits, or production materializer diagnostic consumption. Roll back by deleting the bounded Calves policy helper/config and returning to the prior balanced Calves 4/4 allocation.
   - 2026-07-24 correction to the stale fixture sentence above: the approved representative preferred base is 67 sets, 21 exercises, and one conditionally activated two-set Upper B Triceps top-up when bounded collateral is insufficient. The old 20-exercise/no-top-up description is no longer contract truth.

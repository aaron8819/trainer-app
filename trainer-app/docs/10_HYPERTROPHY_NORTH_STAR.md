...
- Current V2 lane-intent benchmark checkpoint, 2026-06-13: the first ontology/laneSelectionIntent v0 benchmark slice is read-only and candidate-quality-only. `buildV2LaneSelectionIntentBenchmark()` checks high-risk lane jobs for explicit required/preferred/disallowed movement and class semantics, directness, stability/fatigue/loadability preferences, duplicate/fallback policy, identity preservation, and failure meaning. Calf direct, hamstring curl, side-delt direct, rear-delt direct, chest-biased press support, and vertical-pull anchor currently pass the explicitness model; low-axial hip extension remains a diagnostic warning because the planner has not yet separated low-axial posterior-chain support from hinge overload. The taxonomy now recognizes Reverse Hyperextension as `low_axial_hip_extension_anchor`. The future low-axial contract should be a support-coverage lane job for loadable, low-axial, direct-or-high-support hip-extension work with meaningful Glutes stimulus and clean variation preference. It should allow hip-thrust/bridge/pull-through/reverse-hyper style families, disallow true hinge overload, hamstring curl substitution, back-extension/lower-back-heavy closure, and generic glute accessories that do not preserve the hip-extension support job, and define failure as unresolved lane-intent contract ownership rather than a materializer ranking instruction. This is a lane-intent contract work queue, not materializer ranking policy.

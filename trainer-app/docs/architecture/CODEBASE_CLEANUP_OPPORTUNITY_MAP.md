...
| Deprecated readiness route | `/api/session-checkins` route removed after caller migration to canonical readiness submit. | `completed_route_cleanup` | `GenerateFromTemplateCard.tsx` posts to `/api/readiness/submit`; deletion-readiness audit found no production caller and no `SessionCheckIn` table reads for current behavior. | Low | Keep Prisma `SessionCheckIn` table/model until a separate data-retention migration decision. |
...
| `/api/session-checkins` | Deprecated readiness route has been deleted. Historical `SessionCheckIn` persistence remains. | `GenerateFromTemplateCard.tsx` uses `/api/readiness/submit`; deletion-readiness audit found no current table readers. | Yes, route only. | Treat any future `SessionCheckIn` schema/table cleanup as a separate migration/data-retention task. |

...
BEGIN
  IF EXISTS (
    ...
    WHERE namespace.nspname = 'public' AND relation.relname LIKE 'Finisher%'
    UNION ALL
    SELECT 1
    FROM pg_catalog.pg_proc procedure
    JOIN pg_catalog.pg_namespace namespace ON namespace.oid = procedure.pronamespace
    WHERE namespace.nspname = 'public' AND procedure.proname LIKE '%finisher%'
    UNION ALL
    SELECT 1
    ...
    RAISE EXCEPTION
      'Finisher migration cannot run because Finisher schema objects already exist';
  END IF;
END;
$$;
...
BEGIN
  ...
  IF OLD."response" IS NOT NULL
    ...
    AND clock_timestamp() >= OLD."expiresAt"

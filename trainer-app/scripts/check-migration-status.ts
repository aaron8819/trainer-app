    ...
    async (environment) => {
      ...
      try {
        ...
        const report = buildMigrationIntegrityReport({
          ...
          ...(await inspectMigrationDatabase(client)),
        });
        console.log(
          `Migration integrity: checkedIn=${report.chain.checkedIn}, ` +
            `applied=${report.chain.applied}, pending=${report.chain.pending}, ` +
            `incomplete=${report.ledger.incomplete.length}, ` +
            `orderViolations=${report.ledger.orderViolations.length}, ` +
            `checksumsMatched=${report.checksums.matched}, ` +
            ...
            `integrityValid=${report.migrationIntegrityValid}, writes=${report.writes}.`,
        );
        console.log(JSON.stringify(report, null, 2));
        if (!report.migrationIntegrityValid) process.exitCode = 1;
      } finally {

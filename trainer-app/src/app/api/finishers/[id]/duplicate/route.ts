...
import { duplicateFinisherRoutine } from "@/lib/api/finisher-library-service";
...
import { duplicateFinisherRoutineSchema } from "@/lib/validation";
...
export async function POST(
  ...
  const paused = productionWritePauseResponse(
    ...
    "/api/finishers/[id]/duplicate",
  );
  if (paused) return paused;

  const parsed = duplicateFinisherRoutineSchema.safeParse(
  ...
  try {
    ...
    const item = await duplicateFinisherRoutine({
      ownerId: owner.id,
      routineId: id,
      expectedRoutineVersionId: parsed.data.expectedRoutineVersionId,
    });

          ...
          {baselineSummary.evaluatedExercises > 0 ? (
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5 text-sm">
              ...
              <p className="mt-1 text-slate-600">
                Context: {baselineSummary.context} Â· Evaluated: {baselineSummary.evaluatedExercises} Â· Updated:{" "}
                {baselineSummary.updated} Â· Skipped: {baselineSummary.skipped}
              </p>
                  ...
                  {baselineSummary.items.map((item) => (
                    <div key={`${item.exerciseName}-${item.newTopSetWeight}`} className="flex flex-wrap gap-2">
                      ...
                      <span className="text-slate-600">
                        {item.previousTopSetWeight ? `${item.previousTopSetWeight} â†’ ` : ""}
                        {item.newTopSetWeight} lbs Ã— {item.reps}
                      </span>
                    </div>
                  ))}
                </div>
                  ...
                  return (
                    <div key={exercise.id} className="rounded-2xl border border-slate-200 p-5">
                      ...
                      <p className="mt-2 text-xs text-slate-500">
                        Why: {exercise.movementPattern.toLowerCase()} pattern. {stressNote} {loadNote}
                      </p>

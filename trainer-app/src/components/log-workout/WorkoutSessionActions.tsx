...
type WorkoutSessionActionsProps = {
  mode?: "inline" | "finishBar" | "optionsOnly";
  workoutHref: string;
  loggedCount: number;
  totalSets: number;
  completed: boolean;
...
export function WorkoutSessionActions({
  ...
  const resolvedMode = mode ?? (showFinishBar ? "finishBar" : "inline");
  ...
  const optionsSheet = (
    <SlideUpSheet isOpen={optionsOpen} onClose={() => setOptionsOpen(false)} title="Workout options">
      <div className="space-y-4">
        <Link
          className="inline-flex min-h-11 w-full items-center justify-center rounded-full border border-slate-300 px-5 py-2 text-sm font-semibold text-slate-900"
          href={workoutHref}
        >
          View workout
        </Link>

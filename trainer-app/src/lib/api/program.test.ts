...
describe("loadHomeProgramSupport", () => {
  ...
  it("counts actual current-week advancing performed sessions separately from next slot ordinal", async () => {
    ...
    const nonAdvancingCloseout = {
      id: "w-closeout",
      status: "COMPLETED",
      scheduledDate: new Date("2026-03-24T01:00:00.000Z"),
      advancesSplit: false,
      selectionMetadata: makeSelectionMetadata({
    ...
    const supplementalSession = {
      id: "w-supplemental",
      status: "COMPLETED",
      scheduledDate: new Date("2026-03-24T02:00:00.000Z"),
      advancesSplit: false,
      selectionMetadata: makeSelectionMetadata({
  ...
  it("builds the active week checklist from the persisted slot sequence", async () => {
    ...
    expect(result.activeWeekPlan).toMatchObject({
      week: 1,
      source: "mesocycle_slot_sequence",
    });
  ...
  it("marks an existing in-progress active-week workout as resumable", async () => {
    ...
    mocks.workoutFindMany
      .mockResolvedValueOnce([
        {
          id: inProgressLowerA.id,
          status: inProgressLowerA.status,
          sessionIntent: inProgressLowerA.sessionIntent,
          scheduledDate: inProgressLowerA.scheduledDate,
          selectionMetadata: inProgressLowerA.selectionMetadata,
        },
      ])
      .mockResolvedValueOnce([completedUpperA])
      .mockResolvedValueOnce([])
      .mockResolvedValueOnce([completedUpperA, inProgressLowerA]);
  ...
  it("surfaces week-close support only when it matches the active home week", async () => {
    setupDashboardMocks(
      {
    ...
    expect(result.gapFill.targetWeek).toBe(4);
  ...
  it("suppresses a stale prior-week advisory after home advances into a new active week", async () => {
    setupDashboardMocks(
      {
        state: "ACTIVE_DELOAD",
        sessionsPerWeek: 3,
    ...
    expect(result.gapFill.reason).toBe("out_of_scope_for_active_week");
  ...
  it("suppresses a same-week closed row on home", async () => {
    setupDashboardMocks(
      {
        state: "ACTIVE_ACCUMULATION",
        sessionsPerWeek: 3,
  ...
  it("suppresses cross-mesocycle week-close support even if a broader selector returns it", async () => {
    setupDashboardMocks(
      {
        id: "meso-1",
        state: "ACTIVE_ACCUMULATION",
    ...
    expect(result.gapFill.eligible).toBe(false);
    expect(result.gapFill.reason).toBe("out_of_scope_for_active_week");
    expect(result.gapFill.weekCloseId).toBe("wc-foreign");
  });
  ...
  it("returns no home week-close support when there is no active mesocycle", async () => {
    setupDashboardMocks(null);
    mocks.constraintsFindUnique.mockResolvedValue({
      weeklySchedule: ["PUSH", "PULL", "LEGS"],
    });
    ...
    expect(result.gapFill.reason).toBe("no_pending_week_close");
    expect(result.gapFill.weekCloseId).toBeNull();
    expect(mocks.findRelevantWeekCloseForUser).not.toHaveBeenCalled();
  });
});

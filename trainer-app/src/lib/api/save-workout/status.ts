...
export type CompletedWorkoutMetrics = {
  allSetsCount: number;
...
export function resolveFinalStatus(
  input: ResolveFinalStatusInput,
): PersistedStatus {
  if (input.action === "mark_completed") {
    if (
      !input.completedMetrics ||
      input.completedMetrics.effectiveSetCount === 0
    ) {
      throw new Error("WORKOUT_COMPLETION_EMPTY");
    }
    return input.completedMetrics.resolvedSignalSetCount <
      input.completedMetrics.allSetsCount
      ? "PARTIAL"
      : "COMPLETED";
  }
...
function isResolvedLog(log: CompletedSetLog | undefined): boolean {
  return (
    Boolean(log) &&
    (log?.wasSkipped === true ||
      log?.actualReps != null ||
...
function isEffectiveLog(log: CompletedSetLog | undefined): boolean {
  return (
    Boolean(log) &&
    log?.wasSkipped !== true &&
...
export function buildCompletedWorkoutMetrics(
  snapshot: CompletedWorkoutSnapshot,
): CompletedWorkoutMetrics {
  const allSets = snapshot.exercises.flatMap((exercise) => exercise.sets);

  return {
    allSetsCount: allSets.length,
    resolvedSignalSetCount: allSets.filter((set) => isResolvedLog(set.logs[0]))
      .length,

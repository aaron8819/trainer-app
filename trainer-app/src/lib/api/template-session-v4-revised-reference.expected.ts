...
export type ExpectedV4RevisedReferenceExercise = {
  placementId: string;
  exerciseId: string;
  setCount: number;
...
export type ExpectedV4RevisedReferenceCase = {
  week: number;
  ...
  exercises: ExpectedV4RevisedReferenceExercise[];
...
type ExpectedSession = {
  slotId: ExpectedV4RevisedReferenceCase["slotId"];
  focus: ExpectedV4RevisedReferenceCase["focus"];
  placements: ExpectedPlacement[];
};

export const V4_REVISED_REFERENCE_CANONICAL_HASH =
...
export const V4_REVISED_REFERENCE_PLACEMENT_IDS_BY_SLOT = Object.fromEntries(
  expectedSessions.map((session) => [
    session.slotId,
    session.placements.map((placement) => placement.placementId),
  ]),
) as Record<ExpectedV4RevisedReferenceCase["slotId"], string[]>;
...
export const EXPECTED_V4_REVISED_REFERENCE_CASES: ExpectedV4RevisedReferenceCase[] = [1, 2, 3, 4, 5]
  .flatMap((week) =>
    expectedSessions.map((session, sequenceIndex) => {
      ...
      return {
        ...
        omittedPlacementIds,
        provenance: {
          revisionId: "v4-revised-reference-revision-1" as const,
          revision: 1 as const,

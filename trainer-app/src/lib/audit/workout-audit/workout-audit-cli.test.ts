  ...
  it("prints compact V2 plan-quality warning evidence with source attribution", () => {
    ...
    expect(summary).toEqual(
      expect.arrayContaining([
        ...
        "Slot/week watch classification: accepted=1 boundedOwner=0 ownerFix=1 staleNoise=0 blockers=0",
        ...
        "--------------------------------",
        "Status: none-ready evaluated=5 ready=0 stopped=4 watch=1",
        "Recommendation: none-ready top=none owner=none score=n/a",
        "Reason: no candidate has measured owner-specific positive impact with bounded delta, non-regression, acceptance/watch clearance, and seed/runtime/receipt/DB non-consumption",
        "Stop reasons: already_promoted_baseline=1, measured_no_impact=2, missing_bounded_delta=1, stale_noise=1",
        "Next safe action: pivot-to-new-owner-specific-candidate-inventory",
        "Non-consumption: production=no demandOrMaterializer=no seedRuntime=unchanged receipts=unchanged persistence=unchanged",
      ]),
    );
  });
  ...
  it("prints persisted successor replay checks and read-only safety", () => {
    ...
    expect(summary).toEqual(
      expect.arrayContaining([
        ...
        "Bench Press | exact_history | high | exact_history | none | future-week prescription readout | loadSource=exact_history confidence=high caution=none",

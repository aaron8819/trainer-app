  ...
  it("dry-run materializes the full required V2 representative skeleton", () => {
    ...
    expect(
      result.slots.flatMap((slotRow) =>
        slotRow.exercises.filter(
          (exerciseRow) =>
            exerciseRow.setCount >= 5 && !exerciseRow.laneIds.includes("calves"),
        ),
      ),
    ).toEqual([]);
    expect(
  ...
  it("compares materialized baseline and trial plans without executable policy", () => {
    ...
    expect(comparison).toMatchObject({
      ...
      summary: {
        ...
        trialIdentityCount: 2,
        selectedIdentityDelta: 1,
        baselineSetCount: 3,
        trialSetCount: 5,
        totalSetDelta: 2,
        baselineMaterializerBlockerCount: 1,
        trialMaterializerBlockerCount: 0,

...
export function buildMigrationIntegrityReport(input: {
  ...
  const exactChain =
    JSON.stringify(checkedInNames) === JSON.stringify(EXPECTED_MIGRATION_CHAIN);
  const ledgerClean =
    failed.length +
      rolledBack.length +
      incomplete.length +
  ...
  const migrationIntegrityValid =
    ...
    writes === 0;
  const unexpectedMigrations = [...unknown].sort();
  ...
  if (!schemaPreflightValid) blockingReasons.push("schema_drift_detected");
